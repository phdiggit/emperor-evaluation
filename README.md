...
- 第五项 B 测试集准入：五条 rule 已按输入类型与风险分级，32 单元不作为通用默认规模；当前仅 `talent_discovery` 可立即建立 8 个开放开发单元，4 个 sealed 单元须等策略与 Gold 冻结后再开放
...
1. 先为 `talent_discovery` 建立 8 个开放开发单元，覆盖新识别、荐举、跨阵营和不适用反例；按当前耗时优先基线预计 2 次调用、1 个并发波次、纯模型约 107.369 秒和 60,796 token，人工回源与 Gold 审查不在此估算内；
2. 4 个 `talent_discovery` sealed 单元仅作后续计划；开放集调校完成并冻结策略与 Gold 前，授权数量保持为 0；
...
5. 四条后续 rule 全部进入各自开放与 sealed 阶段时，纯模型上界约 16 次调用、8 个顺序波次、858.952 秒和 486,368 token；这不是当前授权任务，也不包含人工史源与 Gold 成本；
