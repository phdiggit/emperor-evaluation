...
- 第二项“治国净收益”已完成秦至清159位评价主体的405分正式结算。当前结构为治理手段165分、治理结果220分、交接质量20分；治理结果由C1民生绝对状态80、C2经济财政绝对状态35、C3社会安全绝对状态60、C4恢复韧性45组成，恢复和K不再在C1/C2/C3重复计分。第二项唯一正式总榜见[`01-第二项治国净收益405分正式结算.md`](docs/评分结算/第二项治国净收益/01-第二项治国净收益405分正式结算.md)；治理手段、治理结果和交接质量分别见[`制度行政/06-治理手段165分正式结算.md`](docs/评分结算/第二项治国净收益/制度行政/06-治理手段165分正式结算.md)、[`财政民生/05-治理结果220分正式结算.md`](docs/评分结算/第二项治国净收益/财政民生/05-治理结果220分正式结算.md)和[`政权交接稳定/02-交接质量20分正式结算.md`](docs/评分结算/第二项治国净收益/政权交接稳定/02-交接质量20分正式结算.md)。A/B1/B2/D1/D3原方向卡继续作为材料裁决和解释入口。规则见[`制度行政合同`](docs/分项规则/第二项治国净收益/制度行政/00-规则与计分合同.md)与[`治理结果合同`](docs/分项规则/第二项治国净收益/财政民生/00-规则与结算合同.md)。当前正式文件写入分数和第二项排名，但不写数据库或跨七大项综合排名。
...
- 第五项已重构为120分三轴：A“履职与认知稳定性”35分、B“人事与权力管理”50分、C“政治克制与品格”35分。既有I5B链路只保留为人事行为候选、回源、责任链和去重基础设施；其五rule加权净信号不再独立映射为大项分数，正式B50必须进入最多四条核心行为链，并受第五项全包最多七条行为链约束。
...
- 既有I5B结果仍是材料诊断信号，不是第五项B50得分、档位或排名；正式投影保持关闭。
...
既有I5B链不得隐式生成第五项B50、档位或排名；只有完成新总合同的全名册行为包和跨时代锚点校准后，正式投影才能开启。
