...
- 第三项“军事与边疆净收益”已完成秦至唐95位评价主体的A/B/C/D及250分合计结算；正式入口位于[`docs/评分结算/第三项军事与边疆净收益/`](docs/评分结算/第三项军事与边疆净收益/README.md)，规则位于[`docs/分项规则/第三项军事与边疆净收益/`](docs/分项规则/第三项军事与边疆净收益/README.md)，共享通鉴卡位于[`docs/共享史料/唐以前编年/`](docs/共享史料/唐以前编年/README.md)。
...
- 唐以后编年史统一采用[`编年体史料通读工作流`](docs/证据规则/编年体史料通读工作流.md)：固定修订正文每卷只完整读取一次，四域共享事实与战役双边登记在同一流程内完成；默认直接在项目 Git 根运行，过程产物写入项目 `tmp/chronicle/**`，验收后的可审计产物发布到 `docs/共享史料/<史书>/**`，未经允许不得迁入会话 worktree；执行采用一个轻量主控加三条无重叠读卷路，并在逐卷预检、五卷审计和全书终审通过前保持零评分、排名和数据库写入。
- 可重建的固定修订原文统一保存在本地 `.cache/source-text/**`，不进入 `docs/` 或 Git；逐卷通读加工件只进入 `tmp/**`，Git 仅保存 `docs/共享史料/**` 的当前中性事实与阅读视图，以及 `docs/公共成果/**` 的已接受 canonical 结果。
