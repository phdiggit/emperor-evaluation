...
- 第三项“军事与边疆净收益”已完成201人A/B/C/D与军事净毁损尾部正式回放。尾部范围0至-40分，只惩罚第三项自身闭合的EN2/EN3、本方C5以上军事成本和本人责任；第二项民生、第四项文明与第五项人物评价不得搬入。正式入口位于[`docs/评分结算/第三项军事与边疆净收益/`](docs/评分结算/第三项军事与边疆净收益/README.md)，规则位于[`docs/分项规则/第三项军事与边疆净收益/`](docs/分项规则/第三项军事与边疆净收益/README.md)。
- 第二项“治国净收益”已有秦至清159位评价主体的正向405分结算快照。当前结构为治理手段165分、治理结果可排序上限220分、交接质量20分；治理结果由C1民生绝对状态80、C2经济财政绝对状态35、C3社会安全绝对状态60、C4社会恢复、承压与可归责恶化组成。C4正向最高45分、负向最低-45分，恢复、恶化和K不在C1/C2/C3重复计分。新增负向分支尚待159人全池回放，现有[`01-第二项治国净收益405分正式结算.md`](docs/评分结算/第二项治国净收益/01-第二项治国净收益405分正式结算.md)不得视为新合同下的最终总榜；治理手段、治理结果和交接质量快照分别见[`制度行政/04-治理手段165分正式结算.md`](docs/评分结算/第二项治国净收益/制度行政/04-治理手段165分正式结算.md)、[`财政民生/05-治理结果220分正式结算.md`](docs/评分结算/第二项治国净收益/财政民生/05-治理结果220分正式结算.md)和[`政权交接稳定/03-交接质量20分正式结算.md`](docs/评分结算/第二项治国净收益/政权交接稳定/03-交接质量20分正式结算.md)。A/B1/B2/D1/D3原方向卡继续作为材料裁决和解释入口。规则见[`制度行政合同`](docs/分项规则/第二项治国净收益/制度行政/00-规则与计分合同.md)与[`治理结果合同`](docs/分项规则/第二项治国净收益/财政民生/00-规则与结算合同.md)。数据库和跨项综合排名写入仍关闭。
- 第四项“文明与国家整合收益”已完成201人、603个皇帝轴的有符号附加项正式结算。A/B/C各为`-22.5—+22.5`，总范围`-67.5—+67.5`；固定池无合格个人增量与观察到的正负相抵均记0，未决证据缺口不得填0。全池候选、空轴碰撞、文化典籍时代相对传播和汉乐府—唐诗—宋词—元曲宏观曲线门禁均已通过。正式入口见[`02-第四项正式结算汇总.md`](docs/评分结算/第四项文明与国家整合收益/02-第四项正式结算汇总.md)，A/B/C三轴逐人证据分别见其“国家共同体与社会整合”“教育可及与人才流动”“文化知识生产传播与生态”子目录；数据库和跨项综合排名仍关闭。
...
- 第二、三、五项共同正向上限为775分，并分别在本项内部保留最低-45、-40、-18的负向尾部；同一损害不得跨项搬分。第四项为范围`-67.5—+67.5`的有符号附加项，不进入共同正向上限分母。第一项条件附加公式中的`M`同步取775，综合式仍为`T = S_common + F + CIV4`。原第六项关键历史决策能力与第七项历史负债均已取消：前者与第一至第五项的决策、反馈和本人素质重复，后者会把各结果项已经消费的负面尾部再次扣分。
- 第五项“统治者政治素质”已完成201人全名册及G-1负向尾部正式回放：A35、B50、C35，199人参评、2人豁免，三轴合计范围-18至120。G-1只能由本人行为链触发，不读取国家崩溃或实际成果规模，短任的高密度完整负向模式也不得因年数短被机械减轻。正式入口见[`04-第五项统治者政治素质正式结算.md`](docs/评分结算/第五项统治者政治素质/04-第五项统治者政治素质正式结算.md)；既有I5B链路只保留为候选、回源、责任链和去重基础设施，数据库和跨项综合排名仍关闭。
...
- 李世民、李渊和刘邦的旧I5B Gold、Episode、REU与加权净信号保留为旧链回归参考；其数量和数值不再作为第五项当前状态，也不得覆盖新总合同下的A/B/C正式结算。
...
- 共享人物画像继续由唯一总登记 `eval/historical_person_profiles/current.{json,md}` 提供，人才总档、分领域档位、全生涯本传凭据和权威校准跨皇帝复用；旧I5B中的团队预算和政治风险只在各皇帝窗口后置投影，不参与第五项新总合同定档。
- 每位皇帝只保留一个 `source-pack.json` 和一组 canonical `result.json` / `result.md`；`result.md` 固定为五项规则的计分详情表，未计分材料显示因子取值。
- 既有I5B结果仍只是材料诊断信号，不是第五项B50得分、档位或排名；第五项正式结算只读取按新总合同验收的人物特质与检索闭合基线。
- 运行入口默认`offline-first`、`report-only`、`shadow-first`；通过独立接受门禁的分项可以发布canonical正式文件和项内排名，但数据库写入与跨项综合排名继续关闭。
...
`first-item-c-registry`只消费本人在统一贡献窗口内的实际战役结果；C1可读取本人实际统筹成果，C2只读取本人前线或一体化指挥，授权、挂名、纯作战设计和他人独立战果不进入C2。窗外败绩不反写统一窗口，窗内可归责败绩按同档负值和C2失败门槛消费。当前生成185人canonical记录，77名统一贡献者适用、108名不适用；证据覆盖不足者明确标为保守下限，不冒充已经证实的零军事贡献。第一项当前正式总表见[`01-第一项创业与政权取得能力正式结算.md`](docs/评分结算/第一项创业与政权取得能力/01-第一项创业与政权取得能力正式结算.md)，解释性分析见[`02-第一项结算总结分析.md`](docs/评分结算/第一项创业与政权取得能力/02-第一项结算总结分析.md)，不写数据库或跨项总排名。

第一项当前原始满分为240分，结构为A100、B60、C80（C1成果50、C2本人前线指挥30）。原始分只在适用奠基人之间比较，不直接加入第二、三、五项的775分共同正向上限分母；综合榜按`F = 0.10 × M × (第一项原始分/240)^1.25`计算条件附加分，再以`T = S_common + F + CIV4`合成。
...
I5B 当前值命令只消费当前 source pack，输出仅用于迁移和筛选人事行为链。旧五rule预算与团队正8、负3结果不得直接进入第五项B50，也不生成第五项档位或排名。
...
`i5b_source_review_quality_probe` 只接受上述精确回源 passage 和人工/主会话审阅后选择的语义档位；decision 不携带数值，所有旧factor数值仍由 `config/i5b-scoring-policy.yml` 确定性映射，用于判断既有材料能否迁入新行为链。每条episode的`ruler_window`必须等于当前皇帝窗口；跨朝实绩不得倒算为当前皇帝功劳。探针输出不再具有独立计分角色，不能生成第五项B50、档位或排名。
...
既有I5B链不得隐式改写第五项B50、档位或排名；后续修订必须重新通过新总合同的全名册行为包、跨时代锚点、组合偏序和确定性导出门禁后，才能覆盖当前正式结算。
