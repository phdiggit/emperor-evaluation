...
- 第三项“军事与边疆净收益”已完成秦至唐95位、五代十国12位及北宋11位完整主政窗口对象的A/B/C/D与250分合计结算，并形成118人统一排名；全部记录均为可复核、可随规则与证据更新的当前值，不设朝代保值例外。正式入口位于[`docs/评分结算/第三项军事与边疆净收益/`](docs/评分结算/第三项军事与边疆净收益/README.md)，规则位于[`docs/分项规则/第三项军事与边疆净收益/`](docs/分项规则/第三项军事与边疆净收益/README.md)。五代十国先由`python -m emperor_v4.evaluation.five_dynasties_third_item`晋升，最终统一入口`python -m emperor_v4.evaluation.north_song_third_item`再原子替换北宋卷001至097的canonical主体阶段卡，并从同一公共登记生成正式分项、合计与118人总榜。
