...
- 第二项“治国净收益”已完成秦至唐95位评价主体的405分正式结算：制度行政四张方向卡为A“制度建设与实际运行”100分、B1“官僚治理与行政执行”100分、B2“反馈纠错与权力约束”80分、D1“继任行政连续性”30分，并按`max(A,B1) + 0.5 × min(A,B1) + B2 + D1`执行重叠折扣形成260分正式分项；再与财政民生130分、政权交接稳定D3 15分确定性合并。第二项唯一正式总榜见[`01-第二项治国净收益405分正式结算.md`](docs/评分结算/第二项治国净收益/01-第二项治国净收益405分正式结算.md)，制度行政260分总表及A/B1/B2/D1四份独立结算位于[`docs/评分结算/第二项治国净收益/制度行政/`](docs/评分结算/第二项治国净收益/制度行政/01-秦至唐95人制度行政260分正式结算.md)；唐以后制度续接统一读取[`重要制度发展节点链`](docs/公共成果/制度行政/03-重要制度发展节点链.md)。规则见[`制度行政合同`](docs/分项规则/第二项治国净收益/制度行政/00-规则与计分合同.md)与[`财政民生合同`](docs/分项规则/第二项治国净收益/财政民生/00-规则与结算合同.md)。当前正式文件写入分数和第二项排名，但不写数据库或跨七大项综合排名。
