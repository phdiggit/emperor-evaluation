...
- 第三项“军事与边疆净收益”已完成秦至唐95位、五代十国12位及北宋11位完整主政窗口对象的A/B/C/D与250分合计结算，并形成118人统一排名；全部记录均为可复核、可随规则与证据更新的当前值，不设朝代保值例外。C/AB只消费去重后的体系压力父任务，低强度战术观察不再增加样本厚度；D逐父周期比较多轴成本收益强度，并以每周期权重、决定性成果密度和负面结构形成归一化风险调整效率。正式入口位于[`docs/评分结算/第三项军事与边疆净收益/`](docs/评分结算/第三项军事与边疆净收益/README.md)，规则位于[`docs/分项规则/第三项军事与边疆净收益/`](docs/分项规则/第三项军事与边疆净收益/README.md)。五代十国先由`python -m emperor_v4.evaluation.five_dynasties_third_item`晋升，最终统一入口`python -m emperor_v4.evaluation.north_song_third_item`再原子替换北宋卷001至097的canonical主体阶段卡，并从同一公共登记生成正式分项、合计与118人总榜。
- 第二项“治国净收益”已完成秦至清159位评价主体的405分正式结算。当前结构为治理手段165分、治理结果220分、交接质量20分；治理结果由C1民生绝对状态80、C2经济财政绝对状态35、C3社会安全绝对状态60、C4恢复韧性45组成，恢复和K不再在C1/C2/C3重复计分。第二项唯一正式总榜见[`01-第二项治国净收益405分正式结算.md`](docs/评分结算/第二项治国净收益/01-第二项治国净收益405分正式结算.md)；治理手段、治理结果和交接质量分别见[`制度行政/04-治理手段165分正式结算.md`](docs/评分结算/第二项治国净收益/制度行政/04-治理手段165分正式结算.md)、[`财政民生/05-治理结果220分正式结算.md`](docs/评分结算/第二项治国净收益/财政民生/05-治理结果220分正式结算.md)和[`政权交接稳定/03-交接质量20分正式结算.md`](docs/评分结算/第二项治国净收益/政权交接稳定/03-交接质量20分正式结算.md)。A/B1/B2/D1/D3原方向卡继续作为材料裁决和解释入口。规则见[`制度行政合同`](docs/分项规则/第二项治国净收益/制度行政/00-规则与计分合同.md)与[`治理结果合同`](docs/分项规则/第二项治国净收益/财政民生/00-规则与结算合同.md)。当前正式文件写入分数和第二项排名，但不写数据库或跨七大项综合排名。
- 第四项“文明与国家整合收益”已完成201人、603个皇帝轴的有符号附加项正式结算。A/B/C各为`-22.5—+22.5`，总范围`-67.5—+67.5`；固定池无合格个人增量与观察到的正负相抵均记0，未决证据缺口不得填0。全池候选、空轴碰撞、文化典籍时代相对传播和汉乐府—唐诗—宋词—元曲宏观曲线门禁均已通过。正式入口见[`01-第四项文明与国家整合收益正式结算.md`](docs/评分结算/第四项文明与国家整合收益/01-第四项文明与国家整合收益正式结算.md)，数据库和跨七项综合排名仍关闭。
...
- 第二、三、五、六项共同正收益上限为865分；第四项改为范围`-67.5—+67.5`的有符号附加项，不进入共同正收益分母。第一项条件附加公式中的`M`同步取865，综合式为`T = S_common + F + CIV4 - D7`。
- 第五项“统治者政治素质”已完成201人全名册正式结算：A“履职与认知稳定性”35分、B“人事与权力管理”50分、C“政治克制与品格”35分，199人进入项内排名，2名无实权或实权极短人物单列豁免。事实特质、完成三层正反复核后的客观皇帝基线和豁免严格分列；正式入口见[`04-第五项统治者政治素质正式结算.md`](docs/评分结算/第五项统治者政治素质/04-第五项统治者政治素质正式结算.md)。既有I5B链路只保留为人事行为候选、回源、责任链和去重基础设施，其五rule加权净信号不再独立映射为B50；数据库和跨七项综合排名仍关闭。
...
- 运行入口默认`offline-first`、`report-only`、`shadow-first`；通过独立接受门禁的分项可以发布canonical正式文件和项内排名，但数据库写入与跨七项综合排名继续关闭。
...
第一项当前原始满分为240分，结构为A100、B60、C80（C1成果50、C2本人前线指挥30）。原始分只在适用奠基人之间比较，不直接加入第二、三、五、六项的865分共同正收益分母；综合榜按`F = 0.10 × M × (第一项原始分/240)^1.25`计算条件附加分，再以`T = S_common + F + CIV4 - D7`合成。
