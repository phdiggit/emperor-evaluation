...
- V3 已退役；V4 是唯一活动实现。
- 战役登记、治理登记、人才等级和政治风险是跨评分项公共基础，统一遵循 [`docs/证据规则/公共成果登记与人物画像规则.md`](docs/证据规则/公共成果登记与人物画像规则.md)；I5B 只消费公共当前值并投影本项因子。执行以朝代为批次：先扫描政书并完成无皇帝窗口的治理底账，再逐个串行运行本朝皇帝；每位皇帝从本纪和臣子列传发现的治理材料先增量合并公共登记，验收后才做窗口绑定和评分。同朝皇帝不并发，因此不设朝代登记锁或协调器。运行时子模型只并发处理当前阶段内部的大批量中性事实或本传 Assertion 草案；公共成果、窗口、人物档位、政治风险、I5B 和 Gold 由连续主会话裁决，成果阶段默认生成 worklist 并等待 `--outcome-review`，不再以子模型重试作为主路径。
...
- 李治已完成首次会话 bootstrap：编年主干只扫描《资治通鉴》卷200—203，不复扫唐初卷184—199；唐代政书统一消费朝代级 `TANG` current，不再建立高宗专用政书副本，完整结果与 Gold 尚待其单皇帝会话生成和审阅。
- 显式指定尚未登记的皇帝时，单皇帝入口会先认领隔离 `bootstrap_required` 会话；身份、窗口、索引或朝代治理 current 未齐只形成可恢复缺口，不再要求旧会话先修改公共 release，也不会以“尚未进入当前链路”直接结束。不同皇帝可在全局4模型槽位内并行 bootstrap。
