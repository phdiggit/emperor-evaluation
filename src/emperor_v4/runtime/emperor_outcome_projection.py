...
5. campaign 必须填写 strategic_result_class、campaign_tier、campaign_tier_basis、land_strategic_value、opponent_strategic_weight、opponent_condition、strategic_stakes、prewar_context、failure_stakes、combat_difficulty、combat_difficulty_basis，并分别填写 operational_costs、objective_shortfalls、attributable_failures；治理字段填 null。prewar_context 只写开战前已经成立的态势，failure_stakes 写若该方向失败的可预见后果，strategic_stakes 取 bounded/major/critical/existential；三者不得用后来战果反向生成。campaign_tier 只按 strategic_result_class 固定映射：local_tactical=C、important_objective=B、major_stage_or_crisis=A、independent_direction=S-、external_hegemony_decisive_defeat/single_pole_or_state_terminal=S、composite_poles_terminal/unification_terminal/external_hegemony_terminal=S+。external_hegemony_decisive_defeat只适用于已经摧毁外部霸权王庭或当次方向主力、形成国家级安全成果，但尚未同时满足统治结构崩解和安全秩序持久逆转的完整胜利；single_pole_or_state_terminal只适用于当时的第一梯队竞争极或具全国分量的外部国家；普通、区域性、衰弱或残余国家不能仅凭灭国进入S。S+只能走后三条终局路径。对手权重只能表示当时竞争体系位置：minor、regional_major、first_tier_pole、dominant_pole、external_state、external_hegemony、unclear。campaign_tier_basis 固定写成“土地轴=<规范值>；对手轴=<战略权重>/<交战状态>；结果轴=<战果>/<目标完成度>，<定级解释>”。作战难度 D0–D4 另行判断且不得改变字母级：D0=无有效抵抗，D1=常规顺风，D2=有重大难点但总体顺风，D3=双方均有现实取胜路径，D4=显著逆风、濒临崩溃或完成逆转；规模、距离和后勤单独最多支持D2，本方失误制造的困难不得抬档。D4还必须说明主帅如何组织、判断并逆转，仅有起始兵力、扩军规模和最终战果不得定D4。正常动员、追击、受伤和敌军来援不得自动算负面。实际兵力或机动力损耗写 operational_costs；目标未完成写 objective_shortfalls；只有原文明示责任时才写 attributable_failures，并逐项给出责任人、责任类型、严重度、依据与逐字引文。不得把背景、作战难度重复登记为战果、成本或失败。
