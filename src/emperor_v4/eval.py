...
from emperor_v4.evaluation.profile_m4_readiness import build as build_profile_m4_readiness
from emperor_v4.evaluation.profile_m4_readiness_verifier import verify as verify_profile_m4_readiness
...
def _parser() -> argparse.ArgumentParser:
    ...
    profile_m4 = commands.add_parser("profile-m4-readiness")
    profile_m4.add_argument("--write", action="store_true")
    commands.add_parser("profile-m4-readiness-verify")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "profile-m3-verify":
        ...
        return 0
    if args.command == "profile-m4-readiness":
        payload = build_profile_m4_readiness(write=args.write)["audit"]
        print(json.dumps({"population_count": payload["population_count"], "summary": payload["summary"]}, ensure_ascii=False, indent=2))
        return 0
    if args.command == "profile-m4-readiness-verify":
        print(json.dumps(verify_profile_m4_readiness(), ensure_ascii=False, indent=2))
        return 0
