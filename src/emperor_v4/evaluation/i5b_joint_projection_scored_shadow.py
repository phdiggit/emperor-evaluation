...
def build_i5b_joint_projection_scored_shadow(
    *,
    ...
    assertion_trace = (
        build_assertion_episode_trace(
            rule_code=rule_code,
            trace_units=material_rows + insufficient_rows,
    ...
    for ruler in sorted(ruler_names):
        ...
        projected_reus = sorted(
            str(material["rule_evidence_unit_ref"])
            for material in material_rows
            if material["ruler"] == ruler
            and material.get("rule_evidence_unit_ref")
        )
        insufficient_reus = sorted(
            str(material["rule_evidence_unit_ref"])
            for material in insufficient_rows
            if material["ruler"] == ruler
            and material.get("rule_evidence_unit_ref")
        )
        traced_reus = sorted(projected_reus + insufficient_reus)
        if traced_reus:
            contribution["rule_evidence_unit_refs"] = traced_reus
            contribution["numerically_projected_rule_evidence_unit_refs"] = projected_reus
            contribution["insufficient_rule_evidence_unit_refs"] = insufficient_reus
