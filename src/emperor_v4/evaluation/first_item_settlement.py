    ...
    *,
    ...
    return {
        ...
        "composite_integration": {
            ...
            "common_positive_denominator": 757,
...
def render_first_item_summary(
    *,
    ...
    lines = [
        ...
        "## 一、结算结论",
        "",
        f"第一项已经完成秦至清全阶段结算：总名册{formal_payload['record_count']}人，其中{formal_payload['eligible_count']}名统一或独立建国主链实际贡献者进入240分正式聚合，{formal_payload['excluded_count']}名普通继承者或非奠基者不适用。完整数值见[第一项正式结算](01-第一项创业与政权取得能力正式结算.md)。A、B、C保留100/60/80原始量尺，分别折算为130/40/70计入；第一项总分不直接加入共同分母，综合榜按`F = 0.10 × M × (第一项总分/240)^1.25`计算奠基人条件附加分。",
        "",
    ...
    lines.extend(
        [
            ...
            "## 五、容易误读的名次",
            "",
            f"- 李世民以{by_name['李世民']['first_item_score_points']:.1f}居首，A原始值{by_name['李世民']['A_score_points']:.1f}与C原始值{by_name['李世民']['C_score_points']:.1f}同时顶尖；B原始值{by_name['李世民']['B_score_points']:.1f}较低，正是本人亲自承担大量主战结果后的归责边界。",
            f"- 朱元璋以{by_name['朱元璋']['first_item_score_points']:.1f}列第二，A原始值{by_name['朱元璋']['A_score_points']:.1f}、B原始值{by_name['朱元璋']['B_score_points']:.1f}、C原始值{by_name['朱元璋']['C_score_points']:.1f}均处高位。",
