...
POLICY_VERSION = "talent-grade-v7-important-calibration-v1"
PRIOR_POLICY_VERSION = "talent-grade-v6-calibration-v1"
VALID_RECOMMENDATIONS = {"top", "important", "usable", "ordinary"}
...
def _promotion_items(*packages: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
    ...
    for package in packages:
        for item in _rows(package.get("items"), "promotion items"):
            ...
            if not person or not isinstance(snapshot, Mapping) or person in result:
                raise ValueError("promotion items require unique person and snapshot")
            result[person] = item
    return result


def build_talent_grade_v7_important_calibration(
    ...
    if prior_calibration.get("policy_version") != PRIOR_POLICY_VERSION:
        raise ValueError("prior talent calibration policy_version mismatch")
    if decision_payload.get("policy_version") != POLICY_VERSION:
        raise ValueError("important calibration policy_version mismatch")
    ...
    current_grades.update(
        {person: str(item["calibrated_grade"]) for person, item in prior_items.items()}
    )
    review_names = {
        person for person, grade in current_grades.items() if grade == "important"
    }

    decisions: dict[str, Mapping[str, Any]] = {}
    for decision in _rows(decision_payload.get("decisions"), "important decisions"):
        ...
        if (
            ...
            or grade not in VALID_RECOMMENDATIONS
            or not basis
        ):
            raise ValueError("important decisions require unique person, valid grade and basis")
    ...
    if set(decisions) != review_names:
        ...
        raise ValueError(f"important calibration coverage mismatch; missing={missing}, extra={extra}")
    ...
    for person in sorted(review_names):
        ...
        if calibrated_grade == "top":
            decision_kind = "upgraded"
        elif calibrated_grade == "important":
            decision_kind = "retained"
        else:
            decision_kind = "downgraded"
        prior_item = prior_items.get(person)
        item = {
            ...
            "gate_matrix": {
                "top_gate_passed": calibrated_grade == "top",
                "important_gate_passed": calibrated_grade in {"top", "important"},
    ...
    report: dict[str, Any] = {
        "schema_version": "talent-grade-v7-important-calibration-report-v1",
