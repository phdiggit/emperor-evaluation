...
from emperor_v4.evaluation.profile_m3_settlement import (
    ACCEPTANCE, AUDIT, CONTRACT, FULL_POOL_REVIEW, HIGH_REVIEW, MANIFEST, MANUAL,
...
def verify_payloads(settlement: dict[str, Any], audit: dict[str, Any], high: dict[str, Any], review: dict[str, Any]) -> dict[str, Any]:
    ...
    assert set(audit["normative_entries"]) == {"SECOND_ITEM_A", "SECOND_ITEM_B2", "SECOND_ITEM_C1", "SECOND_ITEM_C2", "SECOND_ITEM_C3", "SECOND_ITEM_C4", "PROFILE_M1", "PROFILE_M2", "PROFILE_C1", "PROFILE_C2", "PROFILE_C3", "PROFILE_C5", "M3_EXPLICIT_ADJUDICATION"}
    ...
    assert all(row["independent_constraint_class_count"] >= 2 and row["lifecycle_count"] >= 2 for row in high["reviews"])
    ...
    assert all(row["positive_and_negative_checked"] and row["lifecycle_closed"] for row in review["records"])
