...
POLICY_VERSION = "talent-grade-v9-high-tier-calibration-v1"
PRIOR_POLICY_VERSIONS = (
    "talent-grade-v6-calibration-v1",
    "talent-grade-v7-important-calibration-v1",
    "talent-grade-v8-final-calibration-v1",
)
REVIEW_GRADES = {"historic", "top"}
VALID_GRADES = {"historic", "top", "important"}
...
def build_talent_grade_v9_high_tier_calibration(
    ...
    if decision_payload.get("policy_version") != POLICY_VERSION:
        raise ValueError("high-tier calibration policy_version mismatch")
    decisions: dict[str, Mapping[str, Any]] = {}
    for decision in _rows(decision_payload.get("decisions"), "high-tier decisions"):
        person = str(decision.get("person") or "").strip()
        grade = str(decision.get("calibrated_grade") or "").strip()
        current_grade = str(decision.get("current_grade") or "").strip()
        basis = str(decision.get("review_basis") or "").strip()
        lanes = tuple(decision.get("primary_lanes") or ())
        if (
            not person
            or person in decisions
            or grade not in VALID_GRADES
            or current_grade != current_grades.get(person)
            or not basis
            or not lanes
        ):
            raise ValueError("high-tier decisions require valid grade, lane and basis")
    ...
    if set(decisions) != review_names:
        ...
        raise ValueError(f"high-tier calibration coverage mismatch; missing={missing}, extra={extra}")
    ...
    for person in sorted(review_names):
        ...
        item = {
            ...
            "primary_lanes": list(decision["primary_lanes"]),
            "achievement_summary": str(decision.get("achievement_summary") or "").strip(),
            "consistency_summary": str(decision.get("consistency_summary") or "").strip(),
            "legacy_summary": str(decision.get("legacy_summary") or "").strip(),
            "gate_matrix": {
        ...
        if not all(
            item[key]
            for key in ("achievement_summary", "consistency_summary", "legacy_summary")
        ):
            raise ValueError(f"high-tier decision summaries are incomplete: {person}")
    ...
    report: dict[str, Any] = {
        "schema_version": "talent-grade-v9-high-tier-calibration-report-v1",
