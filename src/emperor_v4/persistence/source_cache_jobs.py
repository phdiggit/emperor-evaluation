...
TERMINAL_SOURCE_CACHE_JOB_STATUSES = frozenset(
...
class InMemorySourceCacheJobRepository:
    ...
    def claim(
        ...
        *,
        worker_id: str,
        lease_seconds: int,
    ) -> ClaimedSourceCacheJob | None:
        now = self._clock()
        candidates = [
            job
            for job in self.jobs.values()
            if job.status in RUNNABLE_SOURCE_CACHE_JOB_STATUSES
        ...
        job = sorted(
            candidates,
            key=lambda item: (-item.priority, item.next_attempt_at, item.job_id),
        )[0]
        job.status = "running"
        job.attempt_count += 1
    ...
    def _owned(
        ...
        if (
            ...
            or current.active_run_id != job.run_id
        ):
            raise RuntimeError(
                "Source Cache job lease 已失效或不属于当前 worker"
            )
        if (
    ...
    def fail(
        ...
        *,
        ...
        current.next_attempt_at = self._clock() + timedelta(
...
def _psycopg() -> tuple[Any, Any]:
    try:
        ...
        raise RuntimeError(
            "V4 Source Cache job repository 需要 psycopg"
        ) from exc
...
class PostgresSourceCacheJobRepository:
    def __init__(
        ...
        dsn: str,
        *,
        schema: str = "v4_source_cache",
    ) -> None:
        if not dsn.strip():
            raise ValueError(
                "PostgresSourceCacheJobRepository 需要显式 V4 DSN"
            )
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    INSERT INTO {self.schema}.jobs (
                        job_id, idempotency_key, input_fingerprint,
                        policy_version, request_payload, status,
        ...
        with psycopg.connect(self.dsn) as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    RETURNING job.job_id, job.idempotency_key,
                              job.input_fingerprint, job.request_payload,
    ...
    def _finish(
        ...
        *,
        ...
        terminal = (
            status == "succeeded"
            or not retryable
            or job.attempt_number >= job.max_attempts
        )
        job_status = (
            "succeeded"
            if status == "succeeded"
