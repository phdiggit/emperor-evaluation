...
本 SOP 在制作或修改人物卡时按需使用；仅修改说明文档无需运行出图流程。
...
同一人物的 JSON 是结构稿、提示词与页面的统一制作输入。身份、八轴语义与档位须与 `config/project.yml` 登记的正式画像结算一致；展示措辞不得改变裁决含义或反向修改正式结果。制作数据变化后更新 JSON，重建受影响的结构参考与出图指令；仅修改 SOP 文案不触发重建。
...
首次批量投产前，用朱元璋、刘秀、李世民完成三组双页成图，至少覆盖两种主题和两种布局；通过人工校对后进入批量生产。输入与流程未变化的已通过样例可复用，不在每次任务重新制作。

布局或生成机制有重大变化时复验受影响的代表性样例，覆盖变化的主题与布局；普通人物内容修订只核对受影响数据与页面。页面内容、八轴与雷达点位准确，尺寸与视觉检查通过后即完成；只有具体错误才重出对应页面，不因模型升级自动重做样例或取消校验。
