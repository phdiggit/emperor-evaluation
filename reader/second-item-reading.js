...
(() => {
  ...
  function ensureSecondSummary(record,t){
    ...
    if(summary.dataset.secondRenderKey!==key){summary.innerHTML=`<h2>先看治国结论</h2><p class="second-item-person-conclusion">${takeaway}</p>${rank?`<span class="second-item-rank">${rank}</span>`:""}<div class="second-item-total-grid"><div>制度与行政<b>${fmt(t.methodScore)} / 165</b><small>制度建设、官僚治理与反馈约束</small></div><div>民生与社会<b>${fmt(t.resultScore)} / 202</b><small>民生、经济财政、社会安全与恢复成本</small></div><div>政权交接<b>${fmt(t.handoffScore)} / 20</b><small>行政承接与继承稳定</small></div></div><div class="second-item-equation">${fmt(t.methodScore)} + ${fmt(t.resultScore)} + ${fmt(t.handoffScore)} = <strong>治国成效 ${fmt(t.totalScore)} / 387</strong></div><p class="subline">本项量表理论范围为 -27.5～387。0不是及格线、历史平均或“中性线”。小数位来自统一计分公式，不表示历史判断本身具有同等测量精度。</p>`;summary.dataset.secondRenderKey=key;}
