...
from scripts.dev import retrieval_v2_import_executor as tool
...
class FakeCursor:
    ...
    def execute(self, sql: str, params=None) -> None:
        ...
        if "from retrieval_v2.retrieval_targets" in lowered:
            ...
            self.row = None
            return
        if "from retrieval_v2.rule_contract_rules" in lowered:
            self.rows = [
...
def test_apply_defaults_to_db_backed_dry_run_without_writes(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert not any("insert into retrieval_v2.source_packs" in statement for statement in conn.statements)
...
def test_execute_import_uses_dependency_order_with_fake_connection(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["executed_counts"]["retrieval_v2.source_packs"] == 1
    assert payload["executed_counts"]["retrieval_v2.material_review_queue"] == 1
    insert_statements = [statement for statement in conn.statements if "insert into retrieval_v2." in statement]
    assert "insert into retrieval_v2.source_packs" in insert_statements[0]
...
def test_execute_import_preserves_claim_and_binding_review_status(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["ok"] is True
    claim_params = next(params for sql, params in conn.executions if "insert into retrieval_v2.material_claims" in sql)
