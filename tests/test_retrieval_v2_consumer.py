...
from scripts.dev import retrieval_v2_consumer as tool
...
def test_completion_stage_delegates_to_existing_consumers(monkeypatch) -> None:
    ...
    payload = tool.execute_completion_stage(env_file=None, dsn_env="NEW_DSN", old_dsn_env="OLD_DSN", execute=False)
...
def test_completion_stage_surfaces_identity_and_talent_blockers(monkeypatch) -> None:
    ...
    payload = tool.execute_completion_stage(env_file=None, dsn_env="NEW_DSN", old_dsn_env="OLD_DSN", execute=True)
...
def test_target_scope_cte_supports_accepted_packs() -> None:
    ...
    assert "retrieval_v2.source_packs" in accepted
...
def test_cli_apply_writes_json_and_markdown(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "retrieval_v2 consumer report" in output_md.read_text(encoding="utf-8")
    assert json.loads(capsys.readouterr().out)["ok"] is True
