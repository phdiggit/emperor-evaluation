...
from scripts.dev import retrieval_v3_claim_cache as fs_tool
from scripts.dev import retrieval_v3_claim_cache_pg as tool
...
def test_claim_cache_pg_sql_stays_in_cache_tables() -> None:
    ...
    prompt_source = (Path(tool.__file__).parents[2] / "scripts/dev/retrieval_v3_candidate_prompt.py").read_text(encoding="utf-8")
    ...
    assert "retrieval_v3.claim_cache" in source
    ...
    assert "retrieval_v3.claim_source_slices" in source
    assert "retrieval_v3.claim_evidence" in source
    assert "insert into retrieval_v3.claim_rule_bindings" not in source
    assert "insert into retrieval_v3.target_rule_score_clusters" not in source
