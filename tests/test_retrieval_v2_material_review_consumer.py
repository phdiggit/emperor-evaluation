...
from scripts.dev import retrieval_v2_material_review_consumer as tool
...
class FakeCursor:
    ...
    def execute(self, sql: str, params=None) -> None:
        ...
        if "from retrieval_v2.material_review_queue mrq" in lowered and "where mrq.review_code" in lowered:
            code = params[0]
            row = self.conn.review_rows.get(code)
            self.row = dict(row) if row else None
            self.rows = []
            return
        if "from retrieval_v2.material_review_queue mrq" in lowered:
...
def test_apply_patch_rows_defaults_to_db_backed_dry_run(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert payload["applied_counts"]["retrieval_v2.material_review_queue"] == 1
    assert payload["applied_counts"]["retrieval_v2.claim_rule_bindings"] == 1
    assert conn.rolled_back is True
    assert any("update retrieval_v2.material_review_queue" in statement for statement in conn.statements)
