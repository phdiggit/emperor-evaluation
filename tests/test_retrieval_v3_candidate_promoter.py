...
from scripts.dev import retrieval_v3_candidate_promoter as tool
...
def test_execute_promotions_writes_binding_link_and_resolves_candidate() -> None:
    ...
    assert counts == {
        "retrieval_v3.claim_rule_binding_candidates": 1,
        "retrieval_v3.claim_rule_bindings": 1,
        "retrieval_v3.material_object_links": 1,
    }
    joined = "\n".join(cur.statements)
    assert "insert into retrieval_v3.claim_rule_bindings" in joined
    assert "retrieval_v3.claim_rule_bindings.usable_for_scoring_cluster" in joined
    assert "or excluded.usable_for_scoring_cluster" in joined
    assert "binding_payload = retrieval_v3.claim_rule_bindings.binding_payload || excluded.binding_payload" in joined
    assert "update retrieval_v3.claim_rule_bindings" in joined
...
def test_reconcile_scoring_gates_updates_existing_promoter_bindings() -> None:
    ...
    assert "update retrieval_v3.claim_rule_bindings crb" in sql
