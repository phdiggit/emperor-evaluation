...
def test_agents_contains_markdown_export_guardrails() -> None:
    ...
    assert "人工阅读型 Markdown 导出" in content
    for needle in [
        "展示优化不得改变源数据、评分、定档、排名、warning 语义或裁判结论",
        "人工复核型 Markdown 默认纯 Markdown，不用 HTML details",
        "长字段、裁判说明、相邻项剥离、warning matched_fields 或 linked evidence 不用宽表",
        "`linked_*`、`cross_item_split_signals / 相邻项剥离说明`、warning `matched_fields` 必须全量展示",
