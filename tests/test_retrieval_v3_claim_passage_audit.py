...
from scripts.dev import retrieval_v3_claim_passage_audit as tool
...
def claim_row(**overrides):
    row = {
        ...
        "emperor_name": "刘恒",
        "source_pack_id": 30,
        "source_pack_code": "SPK-I5B-LH-APPOINTMENT-DELEGATION",
...
def test_enqueue_material_reviews_writes_claim_level_queue() -> None:
    ...
    assert counts == {"retrieval_v3.material_review_queue": 1}
    joined = "\n".join(cur.statements)
    assert "insert into retrieval_v3.material_review_queue" in joined
    assert "queue_status = retrieval_v3.material_review_queue.queue_status" in joined
...
def test_enqueue_gap_events_writes_codex_review_repair_signal() -> None:
    ...
    assert counts == {"retrieval_v3.coverage_gap_events": 1}
    joined = "\n".join(cur.statements)
    assert "insert into retrieval_v3.coverage_gap_events" in joined
    assert "then retrieval_v3.coverage_gap_events.status" in joined
