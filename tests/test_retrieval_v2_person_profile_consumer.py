...
from scripts.dev import retrieval_v2_person_profile_consumer as tool
...
def test_build_profile_plan_uses_only_current_person_objects_and_marks_missing() -> None:
    ...
    assert plan["operation_counts"] == {"retrieval_v2.person_profiles": 2}
...
class FakeCursor:
    ...
    def execute(self, sql: str, params=None) -> None:
        ...
        if "from public.raw_objs" in lowered:
            ...
            self.row = None
            return
        if "from retrieval_v2.objects" in lowered:
            self.rows = [dict(row) for row in self.conn.person_rows]
            self.row = None
            return
        if "insert into retrieval_v2.person_profiles" in lowered:
            self.conn.next_id += 1
...
def patch_fake_db(monkeypatch) -> tuple[FakeConnection, FakeConnection]:
    old_conn = FakeConnection(old_rows=[old_talent_row("冯唐", "顶级人才")])
...
def test_apply_defaults_to_db_backed_dry_run_without_writes(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert not any("insert into retrieval_v2.person_profiles" in statement for statement in new_conn.statements)
...
def test_execute_upserts_one_profile_per_current_person_object(monkeypatch) -> None:
    ...
    payload = tool.execute_person_profile_consumer(env_file=None, dsn_env="NEW_DSN", old_dsn_env="OLD_DSN", execute=True)

    assert payload["executed"] is True
    assert payload["write_db"] is True
    assert payload["executed_counts"] == {"retrieval_v2.person_profiles": 2}
    assert new_conn.committed is True
    insert_statements = [statement for statement in new_conn.statements if "insert into retrieval_v2.person_profiles" in statement]
