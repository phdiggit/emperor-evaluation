...
def test_run_judge_round_can_use_deepseek_provider(tmp_path: Path, monkeypatch) -> None:
    captured: dict = {}
    ...
    class FakeResponse:
        ...
        def read(self) -> bytes:
            payload = {
                "choices": [{"message": {"content": '{"status":"succeeded","claims":[]}'}}],
    ...
    monkeypatch.setattr(tool.llm_providers.urllib.request, "urlopen", fake_urlopen)
    prompt_path = tmp_path / "judge_prompt.md"
    prompt_path.write_text("Return JSON", encoding="utf-8")

    result = tool.run_judge_round(
        prompt_path=prompt_path,
        person_dir=tmp_path,
        round_index=0,
        codex_runner=lambda _invocation: pytest.fail("codex should not run"),
        codex_bin="codex",
        timeout_seconds=30,
    ...
    assert result["payload"] == {"status": "succeeded", "claims": []}
...
def test_parse_json_model_content_strips_markdown_fence() -> None:
    assert tool.parse_json_model_content('```json\n{"ok": true}\n```') == {"ok": True}


def test_run_judge_round_writes_deepseek_failure_diagnostics(tmp_path: Path, monkeypatch) -> None:
    class FakeResponse:
        def __enter__(self):
            return self
                ...
                "choices": [
                    {
                        ...
                        "message": {
                            "content": "{\"status\":\"succeeded\",\"claims\":[{\"claim_summary\":\"truncated"
                        },
                    }
                ],
                "usage": {"prompt_tokens": 13, "completion_tokens": 8192},
            }
            return json.dumps(payload).encode("utf-8")

    monkeypatch.setenv("DEEPSEEK_API_KEY", "test-key")
    monkeypatch.setattr(tool.llm_providers.urllib.request, "urlopen", lambda *_args, **_kwargs: FakeResponse())
    prompt_path = tmp_path / "judge_prompt.md"
    prompt_path.write_text("Return JSON", encoding="utf-8")

    with pytest.raises(tool.RetrievalV2CleanRunnerError, match="JSON"):
        tool.run_judge_round(
            prompt_path=prompt_path,
            person_dir=tmp_path,
            round_index=0,
            codex_runner=lambda _invocation: pytest.fail("codex should not run"),
    ...
    failure = json.loads((tmp_path / "judge_last_message.round0.raw_failure.json").read_text(encoding="utf-8"))
    assert failure["provider"] == "deepseek"
    assert failure["phase"] == "judge"
    assert failure["diagnostics"]["finish_reason"] == "length"
    assert failure["diagnostics"]["usage"]["output_tokens"] == 8192
    assert "truncated" in failure["diagnostics"]["content_excerpt"]["head"]
