...
from scripts.dev import retrieval_v3_claim_passage_repair as tool
...
def test_candidate_payload_paths_falls_back_to_latest_target_run(tmp_path: Path, monkeypatch) -> None:
    ...
    old_path = tmp_path / "tmp" / "retrieval_v3_clean_runs" / "runner_old" / "TGT-1_appointment_delegation" / "candidates.final.json"
    new_path = tmp_path / "tmp" / "retrieval_v3_clean_runs" / "runner_new" / "TGT-1_appointment_delegation" / "candidates.final.json"
...
def test_apply_repair_patch_relinks_claim_passages(monkeypatch) -> None:
    ...
    assert payload["applied_counts"]["retrieval_v3.source_passages"] == 1
    assert "delete from retrieval_v3.claim_source_passages" in joined
    assert "insert into retrieval_v3.claim_source_passages" in joined
    assert "update retrieval_v3.material_review_queue" in joined
    assert "update retrieval_v3.coverage_gap_events" in joined
    assert "update retrieval_v3.jobs" in joined
