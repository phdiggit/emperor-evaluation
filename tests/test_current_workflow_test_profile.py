...
def test_current_workflow_manifest_keeps_native_chain_and_excludes_legacy_examples() -> None:
    ...
    assert "test_retrieval_v2_claim_extraction_worker.py" in names
    assert "test_retrieval_v3_candidate_binding_consumer.py" in names
    assert "test_retrieval_v2_factorization_consumer.py" in names
    assert "test_retrieval_v2_rule_scorer.py" in names
