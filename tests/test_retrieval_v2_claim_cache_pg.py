...
def test_claim_cache_pg_sql_stays_in_cache_tables() -> None:
    ...
    assert tool.DEFAULT_ALLOWED_EXTRACTOR_VERSIONS == ("claim_extraction_only:v6_focal_object_owner",)
    ...
    assert issues == [
        {
            ...
            "allowed_extractor_versions": ["claim_extraction_only:v6_focal_object_owner"],
...
def test_filter_prepared_rows_by_run_codes_keeps_related_evidence_and_slices(tmp_path: Path) -> None:
    ...
    new_claim["extractor_version"] = "claim_extraction_only:v6_focal_object_owner"
