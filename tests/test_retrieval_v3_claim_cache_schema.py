...
MIGRATION_PATH = ROOT / "db" / "migrations" / "20260708_retrieval_v3_claim_cache.sql"
...
def test_claim_cache_schema_uses_enums_for_finite_value_fields() -> None:
    ...
    assert enum_types(sql) == {
        "rv3_claim_direction",
        "rv3_object_type",
        "rv3_claim_cache_type",
        "rv3_claim_fact_schema",
        "rv3_claim_cache_status",
        "rv3_claim_support_level",
        "rv3_claim_route_status",
        "rv3_profile_claim_field",
        "rv3_profile_claim_status",
    }
    assert commented_types(sql) == enum_types(sql)
    for enum_column in [
        "claim_type retrieval_v3.rv3_claim_cache_type",
        "fact_schema retrieval_v3.rv3_claim_fact_schema",
        "object_type retrieval_v3.rv3_object_type",
        "status retrieval_v3.rv3_claim_cache_status",
        "support_level retrieval_v3.rv3_claim_support_level",
        "candidate_direction retrieval_v3.rv3_claim_direction",
        "route_status retrieval_v3.rv3_claim_route_status",
        "profile_field retrieval_v3.rv3_profile_claim_field",
        "proposal_status retrieval_v3.rv3_profile_claim_status",
    ]:
...
def test_claim_cache_schema_keeps_cache_separate_from_scoring_tables() -> None:
    ...
    assert "update retrieval_v3." not in sql
    assert "references retrieval_v3.claim_rule_bindings" in sql
    assert "references retrieval_v3.person_profiles" in sql
