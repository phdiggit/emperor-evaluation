...
from scripts.dev import retrieval_v2_object_source_cache as tool
from scripts.dev import retrieval_v2_object_source_cache_hint_worklist as hint_worklist
from scripts.dev import retrieval_v2_object_source_cache_patch as seed_patch
from scripts.dev import retrieval_v2_object_source_cache_seed as seed_tool
...
def test_review_audit_cli_writes_reports(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "dev" / "retrieval_v2_object_source_cache.py"),
            "review-audit",
            "--cache-root",
            str(cache_root),
            "--output-json",
    ...
    assert "# retrieval_v2 object source cache review audit" in (tmp_path / "audit.md").read_text(encoding="utf-8")
...
def test_source_hint_worklist_cli_writes_outputs(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "dev" / "retrieval_v2_object_source_cache_hint_worklist.py"),
            "--cache-root",
            str(cache_root),
            "--output-json",
    ...
    assert "# retrieval_v2 object source hint correction worklist" in (tmp_path / "worklist.md").read_text(encoding="utf-8")
...
def test_seed_patch_cli_writes_output_and_report(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "dev" / "retrieval_v2_object_source_cache_patch.py"),
            "apply",
            "--seed-jsonl",
            str(seed_jsonl),
            "--patch-json",
    ...
    assert "# retrieval_v2 object source cache seed patch report" in (tmp_path / "patch_report.md").read_text(encoding="utf-8")
