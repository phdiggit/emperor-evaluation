...
from scripts.dev import retrieval_v3_clean_cli
from scripts.dev import retrieval_v3_clean_runner as tool
from scripts.dev import retrieval_v3_discovery_profiles
from scripts.dev import retrieval_v3_source_candidates
...
def test_cli_merges_public_emp_metadata_into_task_target_payload() -> None:
    task = {"target_code": "TGT-I5B-CC", "target_payload": {"seed_source": "retrieval_v3_bootstrap"}}
    result = retrieval_v3_clean_cli._with_emp_metadata_target_payload(
        task,
        {"period": "三國", "title": "魏武帝", "ignored": "x"},
    )

    assert result["target_payload"] == {
        "seed_source": "retrieval_v3_bootstrap",
        "period": "三國",
        "title": "魏武帝",
    }
...
def test_cli_loads_query_profile_metadata_for_missing_public_emp() -> None:
    metadata = retrieval_v3_clean_cli._load_query_profile_metadata(["刘娥"])
...
def test_cli_object_source_presearch_keeps_emp_metadata_after_normalize(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        retrieval_v3_clean_cli.taskgen_preseed,
        "expand_task_sources_for_objects",
        lambda task, context, **_: dict(task),
    )
    result = retrieval_v3_clean_cli._expand_object_sources_after_taskgen(
        args=args,
...
def test_cli_can_reuse_discovery_profile_root(tmp_path: Path, capsys, monkeypatch) -> None:
    ...
    profile = retrieval_v3_discovery_profiles.profile_from_task(task_without_alias_gap())
    retrieval_v3_discovery_profiles.write_profile(profile, profile_root)

    monkeypatch.setenv("TEST_RETRIEVAL_V3_DSN", "postgresql://example")
    monkeypatch.setattr(
        tool,
...
def test_streaming_taskgen_pipeline_isolates_single_target_failure(tmp_path: Path) -> None:
    ...
    def flaky_taskgen(invocation: tool.CodexInvocation) -> tool.CodexResult:
        if "刘邦" in invocation.prompt:
            raise tool.RetrievalV3CleanRunnerError("boom")
...
def test_clean_pipeline_overlays_object_source_cache_before_candidates(tmp_path: Path) -> None:
    ...
    retrieval_v3_source_candidates.write_cached_text(
        source_cache,
        "wikisource:宋史/卷999",
        ...
        json.dumps(
            {
                ...
                "retrieval_v3_clean_runs": str(clean_runs_root),
...
def test_cli_claim_cache_stable_rerun_preset_rejects_local_source_cache_fallback(tmp_path: Path) -> None:
    ...
    with pytest.raises(tool.RetrievalV3CleanRunnerError, match="requires --source-cache-root"):
        tool.main(
            [
                "--task",
                str(task_path),
...
def test_cli_judge_shard_workers_defaults_to_four_for_item_wide_shadow() -> None:
    ...
    assert retrieval_v3_clean_cli._effective_judge_shard_workers(args) == 4
...
def test_cli_judge_shard_workers_keeps_non_shadow_default_and_explicit_value() -> None:
    ...
    assert retrieval_v3_clean_cli._effective_judge_shard_workers(normal_args) == 2
    assert retrieval_v3_clean_cli._effective_judge_shard_workers(explicit_args) == 3
...
def test_i5b_item_wide_shadow_context_is_rewritten_before_taskgen() -> None:
    ...
    context = retrieval_v3_clean_cli._with_shadow_context(task_with_candidate_source_gap(), args)
    skeleton = retrieval_v3_clean_cli.task_skeleton.build_task_skeleton(context)
