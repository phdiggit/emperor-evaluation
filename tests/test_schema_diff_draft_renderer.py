...
def test_adr_file_exists_and_check_passes() -> None:
    path = ROOT / "archive" / "docs" / "adr" / "ADR-schema-diff-draft-renderer.md"
...
def test_adr_contains_required_boundaries() -> None:
    content = (ROOT / "archive" / "docs" / "adr" / "ADR-schema-diff-draft-renderer.md").read_text(encoding="utf-8")
...
def _tmp_adr(tmp_path: Path) -> Path:
    ...
    adr.write_text(
        (ROOT / "archive" / "docs" / "adr" / "ADR-schema-diff-draft-renderer.md").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    return adr
