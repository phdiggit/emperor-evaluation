...
def test_reader_does_not_promote_internal_coverage_or_evidence_ceiling_to_public_strength_tags():
    from pathlib import Path
    root = Path(__file__).resolve().parents[1]
    template = (root / "reader/index.template.html").read_text(encoding="utf-8")
    start = template.index("function contextIntensityEntries")
    end = template.index("function contextIntensity(c)", start)
    block = template[start:end]
    assert "['intensity','material_intensity']" in block
    assert "coverage_status" not in block
    assert "evidence_ceiling" not in block
    assert "materialIntensityNames" in template
    assert "材料强度" in template
