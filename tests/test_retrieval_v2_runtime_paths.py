...
from scripts.dev import retrieval_v2_runtime_paths as tool
        ...
        json.dumps(
            {
                ...
                "retrieval_v2_clean_runs": str(path.parent / "active" / "clean_runs"),
                "retrieval_v2_consumption": str(path.parent / "active" / "consumption"),
...
def test_load_runtime_paths_from_config(tmp_path: Path) -> None:
    ...
    assert paths["retrieval_v2_clean_runs"] == tmp_path / "active" / "clean_runs"
    assert paths["retrieval_v2_consumption"] == tmp_path / "active" / "consumption"
...
def test_load_runtime_paths_can_force_local_fallback() -> None:
    ...
    assert paths["retrieval_v2_clean_runs"] == tool.ROOT / "tmp" / "retrieval_v2_clean_runs"
    assert paths["source_cache"] == tool.ROOT / "tmp" / "retrieval_v2_source_cache"
