...
from scripts.dev import retrieval_v3_person_context_consumer as tool
...
class FakeCursor:
    ...
    def execute(self, sql: str, params=None) -> None:
        ...
        if "from public.emps" in lowered:
            ...
            self.row = None
            return
        if "from retrieval_v3.objects" in lowered and "join retrieval_v3.target_objects" not in lowered:
            self.rows = [dict(row) for row in self.conn.person_rows]
            self.row = None
            return
        if "join retrieval_v3.target_objects" in lowered:
            self.rows = [dict(row) for row in self.conn.target_rows]
            self.row = None
            return
        if "insert into retrieval_v3.person_affiliations" in lowered or "insert into retrieval_v3.person_roles" in lowered:
            self.conn.next_id += 1
...
def test_apply_defaults_to_db_backed_dry_run_without_writes(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert not any("insert into retrieval_v3.person_affiliations" in statement for statement in new_conn.statements)
...
def test_execute_writes_affiliations_before_roles(monkeypatch) -> None:
    ...
    payload = tool.execute_person_context_consumer(env_file=None, dsn_env="NEW_DSN", reference_dsn_env="REFERENCE_DSN", execute=True)

    assert payload["executed"] is True
    assert payload["write_db"] is True
    assert payload["executed_counts"] == {
        "retrieval_v3.person_affiliations": 2,
        "retrieval_v3.person_roles": 1,
    }
    assert new_conn.committed is True
    insert_statements = [statement for statement in new_conn.statements if "insert into retrieval_v3." in statement]
    assert "insert into retrieval_v3.person_affiliations" in insert_statements[0]
    assert "insert into retrieval_v3.person_roles" in insert_statements[-1]
