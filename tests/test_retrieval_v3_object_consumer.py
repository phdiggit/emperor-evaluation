...
from scripts.dev import retrieval_v3_object_consumer as tool
...
class FakeCursor:
    ...
    def execute(self, sql: str, params=None) -> None:
        ...
        if "from retrieval_v3.object_resolution_queue" in lowered:
            self.rows = [dict(row) for row in self.conn.queue_rows]
            self.row = None
            return
        if "from retrieval_v3.claim_rule_bindings" in lowered:
...
def test_build_object_plan_blocks_non_single_person_queue() -> None:
    ...
    assert payload["operation_counts"]["retrieval_v3.material_object_links"] == 0
...
def test_build_object_plan_counts_script_variant_name() -> None:
    ...
    assert payload["operation_counts"]["retrieval_v3.object_names"] == 2
...
def test_build_object_plan_accepts_claim_object_fallback_link() -> None:
    ...
    assert payload["operation_counts"]["retrieval_v3.material_object_links"] == 1
...
def test_execute_writes_objects_before_material_links(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["executed_counts"]["retrieval_v3.material_object_links"] == 1
    insert_statements = [statement for statement in conn.statements if "insert into retrieval_v3." in statement]
    assert "insert into retrieval_v3.objects" in insert_statements[0]
    assert "insert into retrieval_v3.material_object_links" in insert_statements[-1]
...
def test_execute_writes_script_variant_name_when_normalized_differs(monkeypatch) -> None:
    ...
    assert payload["executed_counts"]["retrieval_v3.object_names"] == 2
    name_inserts = [statement for statement in conn.statements if "insert into retrieval_v3.object_names" in statement]
