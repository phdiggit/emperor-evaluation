...
from scripts.dev import retrieval_v3_target_person_consumer as tool
...
class FakeCursor:
    ...
    def execute(self, sql: str, params=None) -> None:
        ...
        if "from retrieval_v3.retrieval_targets" in lowered:
            ...
            self.row = None
            return
        if "insert into retrieval_v3." in lowered:
            self.conn.next_id += 1
...
def patch_fake_db(monkeypatch) -> tuple[FakeConnection, FakeConnection]:
    reference_conn = FakeConnection(old=True)
...
def test_apply_defaults_to_db_backed_dry_run_without_writes(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert not any("insert into retrieval_v3.objects" in statement for statement in new_conn.statements)
...
def test_execute_writes_objects_before_profiles_and_roles(monkeypatch) -> None:
    ...
    payload = tool.execute_target_person_consumer(env_file=None, dsn_env="NEW_DSN", reference_dsn_env="REFERENCE_DSN", item_code="I5B", execute=True)

    assert payload["executed"] is True
    assert payload["executed_counts"]["retrieval_v3.objects"] == 1
    assert payload["executed_counts"]["retrieval_v3.person_profiles"] == 1
    assert payload["executed_counts"]["retrieval_v3.person_roles"] == 1
    assert new_conn.committed is True
    inserts = [statement for statement in new_conn.statements if "insert into retrieval_v3." in statement]
    assert "insert into retrieval_v3.objects" in inserts[0]
    assert "insert into retrieval_v3.person_roles" in inserts[-1]
    role_statement = inserts[-1]
    assert "coalesce(nullif(excluded.dynasty_label, ''), retrieval_v3.person_roles.dynasty_label)" in role_statement
    assert "role_payload = retrieval_v3.person_roles.role_payload || excluded.role_payload" in role_statement
