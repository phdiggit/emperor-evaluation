...
from scripts.dev.retrieval_v3_pg_schema import render_sql


ROOT = Path(__file__).resolve().parents[1]
MIGRATION_PATH = ROOT / "db" / "migrations" / "20260708_retrieval_v3_claim_extraction_jobs.sql"
...
def test_claim_extraction_jobs_schema_creates_queue_tables() -> None:
    assert created_tables(migration_sql()) == {
        ...
        "claim_extraction_job_runs",
    }


def test_claim_extraction_jobs_schema_uses_enums() -> None:
    sql = migration_sql()
...
def test_claim_extraction_jobs_schema_comments_every_table_and_column() -> None:
    sql = migration_sql()
...
def test_claim_extraction_jobs_schema_does_not_touch_consumption_tables() -> None:
