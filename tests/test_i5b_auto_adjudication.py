...
def test_export_i5b_auto_adjudication_split_layout_writes_index_and_active_detail_pages() -> None:
    ...
    assert "- **missing_evidence_person_count**：0" in index_content
...
def test_formal_landing_table_reflects_auto_drafts() -> None:
    ...
    assert "朱元璋 | 已有证据 | 可进入正式评分 | 强正受压制，不上探极正 | 强正受压制 | 良好 | 32.15 |" in formal_content
    assert "刘邦 | 已有证据 | 可进入正式评分 | 强正受压制，不上探极正 | 强正受压制 | 良好 | 32.15 |" in formal_content
    assert "雍正 | 已有证据 | 可进入正式评分 | 中正受中负压制 | 中正受中负压制 | 一般 | 23.39 |" in formal_content
    ...
    for person in ["朱棣", "朱瞻基", "朱由检", "皇太极", "玄烨", "弘历"]:
        assert f"{person} | 已有证据 | 可进入正式评分 |" in formal_content
    assert "不出分 | 不排名" not in formal_content
    ...
    assert "- **missing_evidence_person_count**：0" in formal_content
