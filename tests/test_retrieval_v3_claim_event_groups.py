...
from scripts.dev import retrieval_v3_claim_event_groups as tool
...
def test_event_group_fetch_uses_owner_scope_view_without_prompt_cost() -> None:
    source = (tool.ROOT / "scripts/dev/retrieval_v3_claim_event_groups.py").read_text(encoding="utf-8")
    prompt_source = (tool.ROOT / "scripts/dev/retrieval_v3_candidate_prompt.py").read_text(encoding="utf-8")
    ...
    assert "join retrieval_v3.claim_cache cc" not in source
...
class CaptureCursor:
    ...
    def execute(self, sql: str, params=None) -> None:
        ...
        if normalized.startswith("delete from retrieval_v3.claim_event_group_members"):
            self.rowcount = 2
        elif normalized.startswith("delete from retrieval_v3.claim_event_groups"):
            self.rowcount = 1
        else:
            self.rowcount = 0
...
def test_replace_existing_event_groups_deletes_selected_scope_before_rebuild() -> None:
    ...
    assert cur.statements[0].startswith("delete from retrieval_v3.claim_event_group_members")
    assert cur.statements[1].startswith("delete from retrieval_v3.claim_event_groups")
