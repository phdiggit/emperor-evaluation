...
from scripts.dev.retrieval_v3_diagnostics_lib import actions, cli, orchestrator, score_chain, selectors
...
def test_build_next_actions_maps_checks_to_commands() -> None:
    ...
    assert "retrieval_v3_material_review_consumer.py worklist" in rows[0]["next_command"]
    assert "retrieval_v3_factorization_worklists.py worklist" in rows[1]["next_command"]
    assert "retrieval_v3_rule_scorer.py apply" in rows[2]["next_command"]
...
def test_main_report_writes_json_and_markdown(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "# retrieval_v3 diagnostics" in md
...
def test_enrich_score_chain_rejects_cross_emperor_claim_id(monkeypatch) -> None:
    payload = {
        "source": {"kind": "rule_scorer_json"},
        "targets": [{"emperor_name": "李世民", "materials": [{"claim_id": 863}], "top_materials": []}],
    }
    monkeypatch.setattr(
        score_chain,
        "fetch_rows",
        lambda *_args, **_kwargs: [{
            "claim_id": 863,
            "claim_code": "CLM-LIUXIU",
            "claim_summary": "刘秀任用贾复。",
            "claim_emperor": "刘秀",
            "claim_object_name": "贾复",
            "claim_direction": "positive",
            "passages": [],
        }],
    )

    with pytest.raises(score_chain.RetrievalV3DiagnosticsError, match="cross-emperor score detail lineage"):
        score_chain.enrich_score_chain_claim_details(object(), payload)
...
def test_input_rule_scorer_enrichment_keeps_native_claim_schema(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda cursor, payload: {**payload, "native_schema": cursor._schema_name},
    )
    ...
    assert json.loads(output_path.read_text(encoding="utf-8"))["native_schema"] == "retrieval_v3"
...
def test_main_score_chain_writes_json_and_markdown(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    md = output_md.read_text(encoding="utf-8")
    assert "# retrieval_v3 score chain" in md
    assert "萧何" in md
    assert "有效信任" in md
    assert json.loads(capsys.readouterr().out)["ok"] is True
