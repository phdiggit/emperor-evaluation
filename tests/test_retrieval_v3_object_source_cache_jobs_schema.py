...
MIGRATION_PATH = ROOT / "db" / "migrations" / "20260708_retrieval_v3_object_source_cache_jobs.sql"
...
def test_object_source_cache_jobs_schema_uses_enums() -> None:
    ...
    assert enum_types(sql) == {
        "rv3_object_source_cache_job_status",
        "rv3_object_source_cache_run_status",
    }
    assert commented_types(sql) == enum_types(sql)
    assert "status retrieval_v3.rv3_object_source_cache_job_status" in sql
    assert "status retrieval_v3.rv3_object_source_cache_run_status" in sql
