...
第五项B当前正式计分链只认已回源对象、史源、规则、结构化因子和计算明细，不从检索包、搜索日志、批次说明、PR 风险说明或对话记忆取分。`obj_srcs` / `obj_attrs` 是对象事实输入，`evd_cluster_calc_details.calc_detail` 是当前可重放计分明细；`rule_evidence_units`、`rule_evidence_unit_members` 和 `fact_relations` 是规则承载对象影子层，用于审计“rule 到底算谁”和上下文 gap，不直接替代证据簇公式。
...
- 建立或维持表达安全、反馈入口和纠错环境；
- 人才生态的结构性改善或结构性破坏；
- 处置核心人才时，处置对象、过错边界、程序边界、外溢影响和后续补救是否能落到具体材料。
...
以下通用因子定义见 `docs/证据规则/证据簇计算公式.md`，第五项B各 `rule` 公式可直接引用；本文件只说明它们在本项中的职责边界。

`attribution_factor` 用来衡量材料能否归因到皇帝本人或其可控制的任用、授权、保全、表达安全决策。它处理“皇帝归因链”强弱，不处理对象质量、处置严重度或岗位结果。

`source_factor` 用来衡量史源可靠性和事实链清楚程度。单一标准史源、事件链清楚、多源互证等差异都放在这里；各 `rule` 不再另设事实真实性、事实确定度或来源充分性因子。

`context_factor` 用来衡量材料在第五项B语境下的剩余相关度、边界强弱和情境修正。它不替代 `source_factor` 的事实链判断，也不替代各 `rule` 内部的对象质量、信任合理性、授权结果或处置严重度。

使用限制：

- 专用因子不得重复承担史源可靠、事实链清楚、皇帝归因或语境边界职责。
- `obj_srcs.note`、`raw_objs.note` 和因子标签只写对象身份、事件事实和计算口径，不写人物总评分或反推结论。
- 若材料只能通过通用因子解释强弱，应调整通用因子赋值；若材料命中具体 `rule` 的业务结构，才使用专用因子。

### 共享业务因子

以下因子可被多个 `rule` 复用，但不属于证据簇默认因子。它们只衡量本项业务结构，不承担史源可靠、事实链清楚、皇帝归因或语境边界职责。

`object_weight` 用来衡量对象在当前 `rule` 和当前材料中的中心性。它不等同于 `talent_quality_factor`：人才质量评价对象本身的历史能力层级，`object_weight` 评价该对象在本条任用、信任、容谏、保全、处置或人才安全材料中的位置和影响范围。
...
- `object_weight` 不得替代 `source_factor`、`context_factor` 或 `attribution_factor`。
- `object_weight` 不得重复表达 `handling_severity`、`trust_depth`、`authorization_intensity` 或 `office_weight` 已经承担的事件强度和岗位强度。
- 同一对象在不同 `rule` 中可以有不同 `object_weight`，但必须能从该 `rule` 下的材料关系解释。
...
负向计分材料必须绑定被处置、受损或表达安全受压的具体人才；人才群体、人才安全机制、诬陷事件和施害者只能作为上下文或补源材料，不得直接作为本规则计分承载对象。酷吏、近幸、佞臣等施害者本人不得作为本规则的受损对象。
...
- 若发现公式输入材料缺漏或承载对象错挂，应先回到 `obj_srcs`、`obj_attrs`、`evd_cluster_calc_details` 和规则承载影子层修正，再重算本表，并运行 `i5b_health_check.py --fail-on-issue`。
