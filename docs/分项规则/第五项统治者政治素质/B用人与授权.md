...
单一皇帝每轮允许启动新检索的人物入口总数最多为12。人物按上游候选优先级稳定去重后截取；超过上限者必须保留为 `deferred_boundary_candidate`，只有可能改变正8、负3或事件材料结算边界时才进入后续轮次。皇帝政策入口和责任群体不占人物名额；已精确回源的旧合格材料直接复用，也不因复用而消耗新检索名额，但旧材料需要新增人物查询时仍计入上限。不得通过拆分人物焦点、别名或同一人物多个任务绕过总数限制。
...
人才等级取人物画像的当前有效档位，不得由官职、声望或单次战果临时反推。五档的统一事实门槛、权威评价要求和校准程序见“四、建立团队”的“人才能力档位”；本分项只使用该档位，不另行评定人物能力。
...
人才等级是人物跨皇帝窗口共用的能力轴，由**生平实绩**和**权威评价**共同确定。生平实绩决定可达到的最高档，权威评价负责确认同时代位置、纠正归属错误和裁决相邻档边界。政治风险与人才等级正交；政治风险、私德、政治结局、被处死或遭构陷均不得降低能力档位，但与专业能力直接相关的重大失败和履职成本必须进入实绩一致性判断。

跨朝人物只维护一个版本化人才等级，不按朝代或皇帝复制档位。政治风险保存为带时间、责任和损害的独立事件；某皇帝团队的风险档由其窗口内已经暴露的事件派生，后朝事件不得倒灌。人物全生涯最高风险只作人物视图，不直接计分。

人物史源抽取与皇帝评分窗口解耦。人物列传以 canonical person、史源页面或人物段落、史源 revision 和抽取合同版本作为幂等身份，完整保留该史源直接支持的跨政权、跨朝代生涯材料；同一人物—同一史源不得因被多个皇帝引用而重复扫描。皇帝、政权、任职阶段及关系只作为带时间的 PersonContext 或 EpisodeParticipant 保存，不能拆成新的 canonical person。人物材料完成后，再由 RulerPersonSetSnapshot 和 RuleEvidenceUnit 按具体皇帝窗口投影；人才等级可以使用版本化全生涯画像，政治风险只有在当前窗口已经实施、暴露或造成损害时才能进入当前皇帝负池。

成果先按独立责任链合并为成就簇。同一职责链、同一结果的连续步骤和重复同类事项只算一个成就簇；官职、受宠、参会、列名、未实施建议和一般声望不构成成就簇。战役人物只标 `主帅`、`主将`、`副将`、`从攻`；文官治理成果只标 `独占`、`主导`、`参与`。去重和责任边界由成就簇结构表达，不在成果摘要中反复书写限制性说明。

| 档位代码 | 中文档位 | 团队正池值 | 生平实绩门槛 | 权威评价与边界要求 |
| --- | --- | ---: | --- | --- |
| `ordinary` | 普通人才 | 0.35 | 在适用材料已达到充分覆盖后，仍未建立稳定专长、明确实施成果或可验证的正向职责能力 | 不得由“材料缺失”推定普通；必须有权威评价或完整履历材料支持其能力未达到 `usable`，否则记为证据不足而非定档 |
| `usable` | 可用人才 | 0.55 | 具有事实支持的稳定专长，或至少一个范围有限、辅助性但已经实施并取得结果的成就簇 | 权威评价至少不与实绩主线形成强冲突；只有官职和泛称能干不得过档，达到重要职责不等于 `important` |
| `important` | 重要人才 | 0.90 | 至少一个结果清楚的重要规模成就簇，或两个彼此独立、本人中高归属的中等规模成就簇 | 原则上需要至少中等权威共识；若权威评价缺失但直接实绩充分，只能形成低置信候选档，正式冻结前须补齐评价或人工说明例外 |
| `top` | 顶级人才 | 1.20 | 达到国家级、决定性战区、重要制度或领域第一梯队高度，并另有稳定兑现、第二个重大成果或重要方法与遗产之一；本人归属必须高 | 皇帝、史臣或其他权威评价用于校准置信度和相邻档边界，不作为直接实绩充分时的硬上限；单一高峰、共同参与、名望或一项代表作通常不足，未达到时代塑造高度仍不得进入 `historic` |
| `historic` | 历史级人才 | 1.60 | 必须达到时代或文明塑造高度，并通过当前分领域等价路径之一；军事与文官的多次国家级兑现不要求跨时代遗产，只有基础制度或单一作品路径要求长期影响 | 需要清楚的个人归属；皇帝、史臣或其他权威材料只校准高度、归责与置信度，不是直接实绩充分时的硬上限。后世学者评价暂缺时可形成待人工冻结的 shadow 候选，不得用传统标签或史料篇幅代替 |

`historic` 使用 `config/talent-grade-v11-domain-equivalent-historic.yml` 的分领域等价路径。下列路径就是“时代或文明塑造高度”的可执行事实定义，不再叠加一个仅凭传统名声或模型印象判断的抽象高度 Gate；权威评价只校准归责、边界和置信度。不同领域的路径用于消除材料形态偏差，不是叠加加分项。

#### 军事人才

- 常规路径：至少两个彼此独立的国家级战役成果，再加一个独立的区域级以上战役成果；计入成果的人物角色须为 `主帅` 或 `主将`。
- 极强双成果路径：只有两个国家级成果也可成立，但两项角色均须为 `主帅` 或 `主将`，且分别直接决定灭国、统一、国家存亡或整场战争结局。
- 国家级不是“对手具有国号”或“战果数字很大”。战役必须实际决定国家存亡、统一、整场战争结局，或者征服仍具主要区域以上战略分量、且足以改变全国战争态势的政权；击败衰弱残余或普通割据不自动达到国家级。区域级必须实际决定主要战区或重要区域的控制、安全或战略态势。
- 同一战争目标下的连续会战、追击、受降和善后先合并为一个 `independent_campaign_key`；不得把一场战役群拆成多项凑数。重大失败不抹除既有成果，但与计入成果相当的灾难性统帅失败必须进入净一致性复核。
- 军事胜果与跨领域代价分开：已经明确取得的战役或战役群成果，不因并存的外交冲突、授权失调、政治风险或其他领域损害被改写为“没有军事能力成果”；这些代价必须另行登记并参与对应规则和净一致性复核。只有军事目标本身失败、战果不可归责或胜利未形成可观察结果，才排除军事成果。
- 军事路径不要求另有兵法著作、制度遗产或后世影响。战役的中性事实、层级和参与责任登记在 `campaign-registry-v1`，人才画像、第一项 C1/C2 和第三项各自后置投影，不复制战役事件。

#### 文官与治理人才

- 常规路径：至少两个独立国家级治理兑现，再加一个独立的区域级以上治理兑现；国家级成果须有一个 `独占`，或至少两个 `主导`。`参与`只作事实登记，不单独满足升档门槛。
- 国家级治理成果必须实际重构并运行全国性核心治理子系统，或者直接完成国家级危机处置、统一治理、财政法制、组织制度等重大结果。任职时间、在中枢列名、提出但未实施的建议和笼统盛世结果都不计。
- 奠基制度路径：一个责任角色为 `独占` 或 `主导`、跨阶段持续运行的基础性全国制度，再加一个独立国家级成果，或两个独立区域级成果。只有这条路径要求长期持续影响。
- 同一改革的起草、讨论、颁行和初期执行原则上合并为一个成就簇；只有职责、运行机制和结果能够独立成立的模块才可分开。

#### 文化、学术与综合人才

- 单一作品达到 `historic` 是极少数文明奠基例外：必须由本人著成或最终定稿，作品完成且长期成为文明或专业领域的基础范式。集体列名、一般名著、单一当朝影响或后世名气不足。
- 非单一奠基作品路径原则上需要三个独立的国家级或领域第一梯队成果。
- 综合人才须至少在两个独立领域分别达到 `top`，不能用多个跨领域小成果相加替代。

定档按以下顺序执行：

1. 从精确史源中提取并合并独立成就簇，记录规模、本人归属、实现结果、持续性和失败成本；
2. 先按领域登记区域级、国家级或时代塑造级成果，再按 V11 分领域等价路径确定能力上限，不读取旧人才等级；
3. 独立整理权威评价，判断评价来源、时代位置、对象是否确为专业能力及多来源共识；
4. 在实绩上限内裁决相邻档，权威评价不得把未过实绩门槛的人抬档；评价缺失降低置信度，不作为负面能力证据；
5. 与相邻档的同领域标杆进行留一校准，被评价人物不得同时作为自己的标杆；
6. 生成版本化档位、依据、置信度、权威共识和证据覆盖状态，证据不足时保持待复核，不得为团队正池补位而强制定档。

整体分布应呈金字塔形，但只在跨皇帝、去重后的全局人物池中作为校准警报，不作为个人配额。核心臣子、名将或特定皇帝团队本来就是筛选后的高能力样本，不得在单皇帝或单领域内部强制压成金字塔。若全局出现 `top` 或 `historic` 异常膨胀，应复查成果独立性、个人归属和标杆尺度，而不是按比例机械降级；若底部异常膨胀，应复查是否把材料缺失误判为 `ordinary`。

### 2. 政治风险严重度 `political_risk_severity`

政治风险先判断是否达到实质性门槛，再判断严重度。只有本人直接实施、命令、持续放任或以实际责任角色造成的公共损害，以及已经开始实施的严重危害企图，才可成立政治风险。私德恶评、政治失败、被处死、被诬告、普通战败、单次失仪、官位过高、亲属身份或后世知名度均不得单独成立风险。

未达到实质性门槛记为 `below_floor`，适用史源尚未查清记为 `insufficient_evidence`；二者都没有严重度，不得为补满负池强行分档。严重度按已接受事件能够支持的最高档，从 `systemic` 向 `limited` 依次判断；一旦命中即停止，不得在相邻档之间取平均。每一档均隐含“未达到更高档门槛”。

| 档位代码 | 中文档位 | 负池值 | 快速定位条件 | 排除边界 |
| --- | --- | ---: | --- | --- |
| `limited` | 有限风险 | 0.25 | 单次、局部、未致死的公共权力不当行为已经实施但损害轻微；或严重危害企图在形成实质损害前被制止 | 不得存在明确的实质侵夺、拘禁、严重伤害、重复行为或群体损害；否则至少进入 `material` |
| `material` | 实质风险 | 0.55 | 已实现且范围有限的滥权、侵夺、非法拘禁、资源损失或其他明确公共损害；或者同类有限风险重复发生 | 不得达到持续严重伤害、群体性损害或显著治理破坏；否则至少进入 `serious` |
| `serious` | 严重风险 | 0.75 | 严重损害重复或持续发生；对一个可识别群体造成已实现损害；或造成显著但尚未达到国家级中断的公共、军政或治理破坏 | 不得达到城市或人口尺度重大损害、反复重大直接伤害或国家运行中断；否则进入 `major` |
| `major` | 重大风险 | 1.00 | 造成城市或大规模人口损害、反复重大直接伤害、重大区域破坏，或已经实现国家级政权与运行扰动 | 若严重损害已经制度化、跨多个地区持续发生或形成国家系统级长期破坏，进入 `systemic` |
| `systemic` | 系统性风险 | 1.35 | 严重损害被制度化或组织化并持续运行；跨多个地区长期造成严重损害；或对国家制度、政权安全与运行体系造成异常重大的已实现破坏 | 单次重大事件、知名恶行或影响深远但未形成系统运行的事件仍只到 `major` |

分档只使用四类证据：**已实现损害强度、受影响范围、重复或持续程度、制度化程度**。按以下优先顺序裁决：已实现损害高于未遂，直接或命令责任高于一般促成，范围和重复性高于传统评价标签。事件数量和史料详略不得抬档；能力、功绩和人才等级也不得抵消政治风险。

严重度与历史影响范围是两个独立字段。`bounded / regional / national / era_shaping` 只说明影响覆盖到哪里，不能替代上述严重度；例如一次国家级政变企图可以具有 `national` 影响范围，但若未造成相应已实现损害，不得仅凭影响范围判为 `major` 或 `systemic`。

只有风险在当前皇帝窗口内已经暴露，才进入该皇帝的团队负池。后朝事件只能用于核对早期事实，不能倒灌前朝窗口；除非当前窗口已有独立史源证明同一风险行为已经实施、被识别或产生损害。进入负池时使用上述五档值，不再转换为旧 `minor / material / major / historic` 四档。
