...
V3 对照有限因子的智能体观察采用 `appointment-delegation-factor-observation-agent-v4` 语义。档位判断遵守以下边界：
...
- `continuity_factor` 的 `stable` 至少需要同一授权下两个可区分的履职、反馈、监督或损害延续事实；任命加一个最终结果仍不足。`long_term_multi_stage` 至少需要两次可区分的授权、续任、战后再次授职或不同任务许可；同一战役推进、结果演变、失控、纠正和案件扩大不算新授权阶段。
