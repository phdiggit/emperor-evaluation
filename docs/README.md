...
`10-*` 至 `21-*` 记录 G2.5、G2.6 与早期 Core 决策，只在定位历史回归时读取。它们不是活动实施计划，也不得驱动新增字母阶段。

原 `22-*` 至 `30-*` 的 Core/Relation 微阶段文档已由 `31-G3R评分最小充分Relation重解释.md` 取代并从工作树删除；需要追溯 Registry 首次切片、endpoint blocking、双审和细类型失败时使用 Git 历史及对应测试、migration 与 `eval/` artifact。原 G3C—G3H 六份短命阶段文档也已合并进 `31`。
