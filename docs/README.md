...
`10-*` 至 `29-*` 记录 G2.5、G2.6 与早期 Relation 实验，只在定位历史回归时读取。它们不是活动实施计划，也不得驱动新增字母阶段。

原 `30-G3REndpoint样本与裁决明细.md` 的逐条展开属于生成审计物，已从长期文档层移除。原 G3C—G3H 六份短命阶段文档已合并进 `31-G3R评分最小充分Relation重解释.md`。
