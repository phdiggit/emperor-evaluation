...
对任意皇帝的多核心对象 claim 抽取，可在 `retrieval_v2_claim_extraction_worker.py extract-from-candidates` 或 `once --execute` 上显式使用 `--claim-shard-mode owner_aware`。该模式由 `retrieval_v2_claim_shard_planner.py` 在本地按 target owner 的 resolved 且 `owner_anchor_eligible=true` mention、切片质量与通用事件词做 A/B/C/D 分级：A 为 owner 锚点明确的事件，B 为无 owner 锚点但含事件信号的对象传记材料，C 为低信号补充材料，D 为导航、错节或其他不合格材料并只进入审计。它不会调用模型；suppressed/ambiguous 或 ineligible mention 只保留给审计，不能写入 prompt 或 owner rebind；D 类切片不进 prompt。`--judge-shard-size` 控制每 shard 的对象数，`0` 表示尽可能合并以减少模型调用开销；`--claim-shard-max-slices` 与 `--claim-shard-max-chars` 默认均为 `0`，即不因切片数或字符数拆分。应先以 owner-aware feature flag 对照 legacy 记录 wall time、input/output tokens、覆盖和 audit findings，再决定默认 shard 预算；token 预算高于 legacy 不构成拒绝条件。
