...
对候选切片较多的核心 object，可在 claim extraction worker 上显式启用 `--candidate-triage-provider deepseek --candidate-triage-duplicate-text-similarity 0.72`，同时保持 `--judge-provider codex`。DeepSeek 只可建议同一 object、同一 source document 内的重复片段，不能输出优先级或判断 claim、owner、正负向、证据强度；worker 只有在建议指向更早切片、文本相似度达到阈值且没有不同 outcome term 时，才把该重复片段延后，不会裁切独立事件。triage 调用最多等待 180 秒，输出上限为 2048 tokens。每轮在对象运行目录写 `claim_candidate_triage.json`，列出所有 decision 与经机械复核后延后的 slice；原始 candidates 输入仍是可恢复事实源。DeepSeek 响应不完整、结构非法或 API 失败时必须 fail-open：全部切片仍交给 Codex。`--filter-ineligible-slices` 仅在显式传入时才会硬过滤，不能因选择 DeepSeek provider 自动启用。
