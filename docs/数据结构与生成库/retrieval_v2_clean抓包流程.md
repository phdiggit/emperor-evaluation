...
对候选切片较多的核心 object，可在 claim extraction worker 上显式启用 `--candidate-triage-provider deepseek --candidate-triage-max-slices-per-object 2`，同时保持 `--judge-provider codex`。DeepSeek 只给同一 object 内切片打 high/medium/low 优先级和重复簇；worker 保留全部 high，并只把预算内的 medium/low 切片送入 Codex 首轮 prompt，不删除原始候选，也不让 DeepSeek 判断 claim、owner、正负向或证据强度。triage 调用最多等待 180 秒，输出上限为 2048 tokens。每轮在对象运行目录写 `claim_candidate_triage.json`，列出所有 decision 与因 prompt budget 延后的 slice；原始 candidates 输入仍是可恢复事实源。DeepSeek 响应不完整、结构非法或 API 失败时必须 fail-open：全部切片仍交给 Codex。`--filter-ineligible-slices` 仅在显式传入时才会硬过滤，不能因选择 DeepSeek provider 自动启用。
