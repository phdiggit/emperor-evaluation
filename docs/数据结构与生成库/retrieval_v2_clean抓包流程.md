...
负向对象发现范围同时覆盖臣僚与皇子、亲王、藩王等宗室人物；宗室只增加 `actor_scope=royal_clan`、封号别名和宗室史源提示，仍复用同一 object source cache、claim extraction、candidate、binding 与 factorization 链，不建立旁路。显式 `source_document_hints` 可指向公开 OCR 页面；此类来源必须保留原页 URL，并以 `ocr_requires_image_review=true` 标记，OCR 未对照原图前只能触发补抓和 claim 复核。识典古籍页面正文从页面 `window._ROUTER_DATA` 的 `paragraphList` 提取，不能用普通 HTML 去标签结果冒充正文；若提取不到段落，必须保持零切片。来源提示显式提供 `ocr_aliases` 时，该来源内必须用其覆盖普通人物别名，避免正确别名在其他章节的偶然提及抢先生成错位切片；若同时提供 `section_start_aliases`，切片起点不得越过最近的本节标题。当前《御制纪非录》只作为朱元璋宗室候选的优先提示，不扩散为所有明代人物的通用来源。
