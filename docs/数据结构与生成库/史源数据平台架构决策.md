...
结论：以 PostgreSQL + RabbitMQ + PostgreSQL FTS 起步；JSONL 短期保留但不继续无限扩张。

本文只确定目标架构和迁移边界，不实现数据库 schema、队列、worker、抓取器或搜索引擎。当前仓库的 JSONL、SQLite 和 Markdown 导出继续服务试点与验证；未来新增史源全文、段落、检索任务和复核状态时，应向数据平台模型收束，而不是继续扩张平行 JSONL 文件。
...
本文只描述目标表方向，不定义 schema。第一版 schema 草案以《史源数据平台实现设计.md》为准。
...
当前 JSONL 继续保留，短期仍作为事实源、导入来源、小样本 fixture 和 PR 审阅载体。现有 `data/*.jsonl`、`data/*_batches/*.jsonl` 和主表字段规范不在本文中迁移或删除。
...
未来切库必须走单向阶段：JSONL 唯一写源、导入 PostgreSQL staging 反复验证、冻结 JSONL 写入、全量导入并校验行数 / ID / 引用 / hash、PostgreSQL 成为唯一写源、JSONL 只作导出快照 / 小 fixture / 历史审计。本文不改动 `data/*.jsonl`，不迁移 batch，不删除过渡文件，也不改变当前生成链路。

## 8. 配置边界

沿用 #227 后的新口径：`project_config.yml` 只控制 active subitem、groups、defaults，以及少量不承载业务事实的本地工具开关。短名单可直接写 `persons`，长清单用 `persons_ref` 或后续 `*_ref` 外置。
...
| --- | --- | --- | --- |
| PostgreSQL | 目标主库 | 强关系、事务、外键、唯一约束、JSONB、FTS、审计链清晰 | 当前即作为主库优先方向；第一阶段先做架构决策和最小原型，不在本 PR 实现 |
