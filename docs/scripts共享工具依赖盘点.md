...
本文件用于 shared 分层治理，记录共享工具依赖盘点、目录规划和迁移规则说明。#193 已开始实迁第一期，只迁移 `export_md_scaffold.py`。
...
`export_md_scaffold.py` 已迁移到 `scripts/shared/`，旧路径保留兼容 wrapper。其他共享工具仍暂留 `scripts/` 根目录。
