...
`scripts/export/export_md.py` 默认只运行 `main` profile，生成少数综合入口文档。子项细节、试点 batch、审计视图、自动结算必须显式指定 profile 或调用专用脚本；旧全量导出行为使用 `python scripts/export/export_md.py --profile all`。
