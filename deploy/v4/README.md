...
四个 release 使用同一套不可变目录规则。Source Cache 负责中央史源预热且 timer 开启；Claim Extractor 的数据库 worker 仍关闭；Dynasty Governance 保持定时运行；Emperor Rebuild release 只供单皇帝主控会话读取，不安装 queue、worker 或 timer：
...
- `emperor-v4-dynasty-governance-worker.timer`：朝代治理中性材料，当前开启；
