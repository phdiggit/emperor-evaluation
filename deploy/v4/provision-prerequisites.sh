...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 RELEASE_SHA UPLOAD_ROOT CODEX_SOURCE SOURCE_PLAN" >&2
  exit 2
fi
...
[[ -f $source_plan ]] || { echo "source plan is missing" >&2; exit 2; }
...
install -d -o root -g emperor-v4 -m 0750 "$etc_root"
...
install -o root -g emperor-v4 -m 0640 "$source_plan" "$etc_root/source-cache-plan.yml"
