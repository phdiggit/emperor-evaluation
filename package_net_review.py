...
PROFILE_CONTRACT_SOURCES = (
    ...
    "docs/分项规则/人物画像轴/M5-组织驾驭与执行编排.md",
...
def _build_profile_contract_document(root: Path) -> bytes:
    ...
    text = dedent(
        ...
        | M5 组织驾驭与执行编排 | 能否拆解任务、配置责任中心并协调监督与恢复，使组织持续交付？ | 个人勤政、命令数量或他轴成果直接换分。 |
        ...
        - **M5**：评价独立组织命题的责任配置、协调监督、持续交付与受阻恢复；不重复消费C3识才授权或C4制度设计命题，详见M5完整合同。
        ...
        4. 所有输出保持离线、确定性、零模型、零网络、零数据库写入；画像正式结果不写入净收益综合榜，也不得由榜单或名望反推。
