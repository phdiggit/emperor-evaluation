alter table retrieval_v3.claim_rule_binding_candidates
...
begin
    if not exists (
        select 1
          from pg_constraint
         where conname = 'rv3_claim_rule_binding_candidates_hint_status_ck'
           and conrelid = 'retrieval_v3.claim_rule_binding_candidates'::regclass
    ) then
        alter table retrieval_v3.claim_rule_binding_candidates
            add constraint rv3_claim_rule_binding_candidates_hint_status_ck
            check (hint_status in ('formal_candidate', 'current_rule_candidate', 'future_rule_hint', 'context_only', 'rejected'));
    end if;
    if not exists (
        select 1
          from pg_constraint
         where conname = 'rv3_claim_rule_binding_candidates_required_facts_present_ck'
           and conrelid = 'retrieval_v3.claim_rule_binding_candidates'::regclass
    ) then
        alter table retrieval_v3.claim_rule_binding_candidates
            add constraint rv3_claim_rule_binding_candidates_required_facts_present_ck
            check (jsonb_typeof(required_facts_present) in ('object', 'array'));
    end if;
end $$;

create index if not exists rv3_claim_rule_binding_candidates_lane_idx
on retrieval_v3.claim_rule_binding_candidates(candidate_item_code, candidate_lane, hint_status, candidate_rule_code);

create index if not exists rv3_claim_rule_binding_candidates_future_hint_idx
on retrieval_v3.claim_rule_binding_candidates(candidate_item_code, candidate_lane, candidate_rule_code, created_at)
where hint_status = 'future_rule_hint';

comment on column retrieval_v3.claim_rule_binding_candidates.candidate_lane is '跨项候选 lane，例如 I5B.team_building 或 I5C.power_control；未知时留空或按 rule_code 回退。';
comment on column retrieval_v3.claim_rule_binding_candidates.hint_status is '候选性质：formal_candidate/current_rule_candidate 可进入窄验；future_rule_hint/context_only/rejected 只能沉淀或复核。';
comment on column retrieval_v3.claim_rule_binding_candidates.required_facts_present is 'judge 输出的候选事实完整性标记；仅供路由和复核，不直接入分。';
comment on column retrieval_v3.claim_rule_binding_candidates.routed_by_profile is '产生候选路由的抓包 profile 或工具名，例如 personnel_political_wide。';
