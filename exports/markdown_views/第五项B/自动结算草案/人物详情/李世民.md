...
* **是否有强负核心（has_strong_negative_core）**：False
* **是否有极强负核心（has_extreme_negative_core）**：False
* **负向边界档（negative_boundary_tier）**：weak_to_medium
* **负向边界是否阻断（negative_boundary_blocking）**：False
* **是否需要相邻项剥离（cross_item_split_required）**：True
* **剥离后剩余强度（cross_item_split_residual_level）**：medium
