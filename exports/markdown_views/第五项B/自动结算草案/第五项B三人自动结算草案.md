...
| 人物（person） | 自动结算方向（auto_band_direction） | 置信度（confidence） | 负向边界档（negative_boundary_tier） | 是否阻断（negative_boundary_blocking_digest） | 规则敏感点（rule_sensitive_points） |
| --- | --- | --- | --- | --- | --- |
| 李世民 | 高位强正，上探极正候选 | 高偏中 | 弱至中 | 否 | 弱负上调中负边界 |
| 刘秀 | 强正受压制，不上探极正 | 中偏高 | 中至强 | 是 | 中负上调强负边界；强负核心压制强正 |
| 刘庄 | 中正受中负压制 | 中 | 相邻项剥离后中度剩余 | 否 | 相邻项主导剥离；B项剩余默认中负 |
