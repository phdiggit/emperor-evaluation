...
| person | positive_cluster_ids | negative_cluster_ids | auto_band_direction | confidence | rule_sensitive_points |
| --- | --- | --- | --- | --- | --- |
| 李世民 | ADJ-I5B-LISHIMIN-POS-TALENT-ECOSYSTEM-001 | ADJ-I5B-LISHIMIN-NEG-TALENT-RISK-001 | 高位强正，上探极正候选 | high | 中负边界是否阻断极正；同一对象正负反转是否单列 |
| 刘秀 | ADJ-I5B-LIUXIU-POS-TALENT-AUTHORIZATION-001 | ADJ-I5B-LIUXIU-NEG-REMONSTRANCE-SAFETY-001 | 强正受压制，不上探极正 | medium_high | 单一维度强正能否极正；强负核心是否压制强正 |
| 刘庄 | ADJ-I5B-LIUZHUANG-POS-TALENT-AUTHORIZATION-001 | ADJ-I5B-LIUZHUANG-NEG-TALENT-SAFETY-001 | 中正受中负压制 | medium | 相邻项强负剥离后的 B 项剩余；中正正证能否机械抵消强负 |
...
- 中负边界是否阻断极正：否；魏征线带 mitigation_flag / upper_bound_flag，默认不阻断。
- 同一对象正负反转是否单列：是；只在证据组层与定档层备注，不把单证继续抬成强负核心。
...
- 单一维度强正能否极正：否；正向仍偏创业期军政授权，默认强正封顶。
- 强负核心是否压制强正：是；表达安全硬证默认触发高档位上探拦截。
...
- 相邻项强负剥离后的 B 项剩余：默认中负剩余；若出现直接寒蝉硬证再升级为强负核心。
- 中正正证能否机械抵消强负：否；中正正证只保留底盘，不机械抵消相邻项强负。
