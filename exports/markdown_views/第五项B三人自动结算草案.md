...
* **命中词**：强正、上探
* **命中字段**：linked_cards[0].scoring_effect、linked_cards[0].evidence_role、linked_cards[3].scoring_effect……（共12项）

**2. 单证不足提示（single_evidence_limit｜I5B-CLUSTER-WARN-SINGLE-EVIDENCE-LIMIT）**

* **提示语**：提示人工检查单条证据是否不足以支撑高强度证据簇，不能机械升档。
* **命中词**：单证
* **命中字段**：linked_cards[2].scoring_effect
...
* **提示语**：提示人工对强证、极端证和上探候选进行回源核验，避免仅凭标签或摘要定性。
* **命中词**：强正、极正、上探
* **命中字段**：cluster.note、cluster.summary、linked_cards[3].scoring_effect……（共11项）

**2. 回源核验提示（source_review_required｜I5B-CLUSTER-WARN-SOURCE-REVIEW-REQUIRED）**

* **提示语**：提示人工对强证、极端证和上探候选进行回源核验，避免仅凭标签或摘要定性。
* **命中词**：强负、极负、上探
* **命中字段**：cluster.summary、linked_cards[0].scoring_effect、linked_cards[0].evidence_role……（共13项）
...
* **提示语**：提示人工对强证、极端证和上探候选进行回源核验，避免仅凭标签或摘要定性。
* **命中词**：强负、极负、上探
* **命中字段**：cluster.note、cluster.summary、linked_cards[0].scoring_effect……（共6项）
