...
- 当前模块状态、retired wrapper 审计记录、审计文档和专属测试以 `docs/治理规则/scripts_registry.json` 为准，本文件不维护项目状态日志。
...
- `docs/治理规则/scripts_registry.json` 是当前实现路径、迁移状态、retired legacy wrapper、root exception、audit docs 和 required tests 的机器可读事实源。
- `docs/治理规则/README.md` 说明规则分类；新增规则先按该表决定写入位置。
