...
from scripts.dev.retrieval_v2_contracts import SOURCE_HINT_TEXT_ALIASES, alias_script_variants, unique_strings
...
left join retrieval_v2.object_names onm on onm.object_id = o.id
left join retrieval_v2.target_objects tob on tob.object_id = o.id and tob.review_status in ('pending', 'accepted')
left join retrieval_v2.retrieval_targets rt on rt.id = tob.target_id
...
def seed_audit_report(seeds: Sequence[Mapping[str, Any]], *, max_issue_rows: int = 100) -> dict[str, Any]:
    ...
    return {
        "schema": "retrieval_v2_object_source_cache_seed_audit_v1",
        "totals": totals,
...
def render_seed_audit_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# retrieval_v2 object source cache seed audit",
        "",
        "## Summary",
        "",
    ...
    *,
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            use_retrieval_v2 = source == "retrieval-v2" or (source == "auto" and relation_available(cur, "retrieval_v2.objects"))
            if source == "retrieval-v2" and not relation_available(cur, "retrieval_v2.objects"):
                raise ObjectSourceCacheSeedError("retrieval_v2.objects table is not available")
            if not use_retrieval_v2 and not relation_available(cur, "raw_objs"):
                raise ObjectSourceCacheSeedError("raw_objs table is not available")
            sql = PERSON_SEEDS_SQL if use_retrieval_v2 else OBJECT_POOL_PERSON_SEEDS_SQL
            seed_source = "retrieval_v2.objects" if use_retrieval_v2 else "raw_objs"
            params: list[Any] = []
            if limit > 0:
                sql += "\nlimit %s"
                params.append(limit)
