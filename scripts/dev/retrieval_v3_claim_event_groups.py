    ...
    *,
    ...
    cur.execute(
        ...
        select
          ...
          from retrieval_v3.claim_atomic_facts c
          join retrieval_v3.claim_owner_scopes os on os.claim_key = c.claim_key
...
def delete_excluded_owner_scope_event_groups(cur: Any, owner_scopes: Sequence[str]) -> int:
    ...
    cur.execute(
        """
        delete from retrieval_v3.claim_event_groups g
         where (
            case
...
def replace_existing_event_groups(cur: Any, *, owner_scopes: Sequence[str], emperor_names: Sequence[str]) -> dict[str, int]:
    ...
    cur.execute(
        f"""
        delete from retrieval_v3.claim_event_group_members m
         using retrieval_v3.claim_event_groups g
...
def upsert_event_group(cur: Any, row: Mapping[str, Any]) -> None:
    cur.execute(
        """
        insert into retrieval_v3.claim_event_groups (
...
def upsert_event_group_member(cur: Any, row: Mapping[str, Any]) -> None:
    cur.execute(
        """
        insert into retrieval_v3.claim_event_group_members (
...
def event_group_report(
    *,
    ...
    return {
        "ok": True,
        "generated_by": "scripts/dev/retrieval_v3_claim_event_groups.py",
