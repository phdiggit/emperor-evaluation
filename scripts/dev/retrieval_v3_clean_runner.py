...
if __name__ == "__main__":
    sys.modules.setdefault("scripts.dev.retrieval_v3_clean_runner", sys.modules[__name__])

from scripts.dev import retrieval_v3_alias_refiner as alias_refiner
from scripts.dev import retrieval_v3_candidate_source_refiner as candidate_source_refiner
from scripts.dev import retrieval_v3_candidate_prompt as candidate_prompt
from scripts.dev import retrieval_v3_claim_cache as claim_cache
from scripts.dev import retrieval_v3_judge_shards as judge_shards
from scripts.dev import retrieval_v3_object_source_cache as object_source_cache
from scripts.dev.retrieval_v3_clean_summary import build_batch_summary, sum_usage, summarize_person
from scripts.dev.retrieval_v3_run_events import RunEventLogger
from scripts.dev import retrieval_v3_source_candidates as source_candidates
from scripts.dev import retrieval_v3_task_skeleton as task_skeleton
from scripts.shared import agent_runtime_config  # noqa: E402

DEFAULT_TARGET_DSN_ENV = "EMPEROR_EVAL_RETRIEVAL_V3_DSN"

class RetrievalV3CleanRunnerError(RuntimeError):
    pass
...
CODEX_SANDBOX_ENV = "RETRIEVAL_V3_CODEX_SANDBOX"
CODEX_ADD_DIRS_ENV = "RETRIEVAL_V3_CODEX_ADD_DIRS"
CODEX_BIN_ENV = "CODEX_BIN"
CODEX_MODEL_ENV = "RETRIEVAL_V3_CODEX_MODEL"
CODEX_REASONING_EFFORT_ENV = "RETRIEVAL_V3_CODEX_REASONING_EFFORT"
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RetrievalV3CleanRunnerError(f"expected object JSON: {path}")
    return payload


def load_env_file(path: Path) -> list[str]:
    if not path.exists():
        raise RetrievalV3CleanRunnerError(f"env file missing: {path}")
    loaded: list[str] = []
...
def resolve_dsn(env_name: str) -> str:
    ...
    if not value:
        raise RetrievalV3CleanRunnerError(f"missing PostgreSQL DSN env var: {env_name}")
    return value
...
def import_psycopg():
    try:
        ...
        raise RetrievalV3CleanRunnerError("psycopg is required for live retrieval_v3 task generation") from exc
...
def extract_json(text: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RetrievalV3CleanRunnerError("Codex output is not a JSON object")
    return payload
...
def run_codex(invocation: CodexInvocation) -> CodexResult:
    ...
    if completed.returncode != 0:
        raise RetrievalV3CleanRunnerError(
...
def fetch_retrieval_contexts(
    *,
        ...
        select
            ...
            coalesce((
                select jsonb_agg(
                  ...
                  from retrieval_v3.target_aliases ta
                 where ta.target_id = t.id
                   and ta.status = 'active'
            ), '[]'::jsonb) as target_aliases
          from retrieval_v3.retrieval_targets t
          join retrieval_v3.rule_contracts rc on rc.id = t.contract_id
          join retrieval_v3.retrieval_intents ri on ri.target_id = t.id
          join retrieval_v3.rule_contract_rules crr on crr.id = ri.contract_rule_id
    ...
    if missing:
        raise RetrievalV3CleanRunnerError(f"missing retrieval_v3 targets for {rule_code}: {missing}")
    return by_name
...
def run_taskgen(
    *,
    ...
    if discovery_profile is not None:
        ...
        if issues:
            raise RetrievalV3CleanRunnerError(f"discovery profile produced invalid task: {issues}")
    ...
    issues = task_skeleton.validate_task_for_candidates(task)
    if issues:
        raise RetrievalV3CleanRunnerError(f"taskgen discovery produced invalid task: {issues}")
    atomic_write_json(person_dir / "task.generated.json", task)
    atomic_write_json(person_dir / "discovery_profile.generated.json", task_skeleton.discovery_profile_from_task(task))
    if event_logger is not None:
        event_logger.emit(
...
def process_task(
    *,
    ...
    def apply_source_refine(stage: str, payload: Mapping[str, Any], object_names: Sequence[str], round_index: int):
        ...
        if issues:
            raise RetrievalV3CleanRunnerError(f"{stage} source refinement produced invalid task: {issues}")
...
def run_streaming_taskgen_pipeline(**kwargs: Any) -> dict[str, Any]:
    from scripts.dev.retrieval_v3_clean_cli import run_streaming_taskgen_pipeline as cli_streaming
    return cli_streaming(**kwargs)


def main(argv: Sequence[str] | None = None) -> int:
    from scripts.dev.retrieval_v3_clean_cli import main as cli_main
...
if __name__ == "__main__":  # pragma: no cover
    ...
    except RetrievalV3CleanRunnerError as exc:
