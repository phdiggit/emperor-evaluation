...
def _local_paths() -> dict[str, Any]:
    ...
    return {
        ...
        "active_root_linux": "",
        "archive_root_linux": "",
        "retrieval_v3_clean_runs": active_root / "retrieval_v3_clean_runs",
        "retrieval_v3_consumption": active_root / "retrieval_v3_consumption",
        "retrieval_v3_feedback": active_root / "retrieval_v3_feedback",
        "retrieval_v3_reports": active_root / "retrieval_v3_reports",
        "source_cache": active_root / "retrieval_v3_source_cache",
    }
...
def _paths_from_config(payload: Mapping[str, Any], *, config_source: str) -> dict[str, Any]:
    ...
    return {
        ...
        "retrieval_v3_clean_runs": _path_from(payload.get("retrieval_v3_clean_runs"))
        or active_root
        / "retrieval_v3_clean_runs",
        "retrieval_v3_consumption": _path_from(payload.get("retrieval_v3_consumption"))
        or active_root
        / "retrieval_v3_consumption",
        "retrieval_v3_feedback": _path_from(payload.get("retrieval_v3_feedback")) or active_root / "retrieval_v3_feedback",
        "retrieval_v3_reports": _path_from(payload.get("retrieval_v3_reports")) or active_root / "retrieval_v3_reports",
...
def default_run_root(name: str, paths: Mapping[str, Any] | None = None) -> Path:
    ...
    return Path(resolved["retrieval_v3_clean_runs"]) / sanitize_run_name(name)


def default_consumption_root(name: str, paths: Mapping[str, Any] | None = None) -> Path:
    resolved = load_runtime_paths() if paths is None else paths
    return Path(resolved["retrieval_v3_consumption"]) / sanitize_run_name(name)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Resolve retrieval_v3 runtime asset paths.")
    ...
    new_run = subparsers.add_parser("new-run", help="Print standard paths for a new retrieval_v3 run.")
