...
from scripts.dev.retrieval_v3_import_plan import ImportPlanError, write_json  # noqa: E402
from scripts.dev import retrieval_v3_object_consumer as object_consumer  # noqa: E402
from scripts.dev import retrieval_v3_person_context_consumer as context_consumer  # noqa: E402
from scripts.dev import retrieval_v3_person_profile_consumer as profile_consumer  # noqa: E402
from scripts.dev import retrieval_v3_target_person_consumer as target_person_consumer  # noqa: E402
...
REASON_CATALOG: dict[str, dict[str, str]] = {
    ...
    "rule_score_required": {
        "owner": "agent_or_human",
        "severity_default": "downstream",
        "description": "已因子化材料还需生成规则信号聚合；先跑 retrieval_v3_rule_scorer，再进入跨 rule 汇总。",
    },
...
def execute_completion_stage(*, env_file: Path | None, dsn_env: str, reference_dsn_env: str, execute: bool) -> dict[str, Any]:
    target_report = target_person_consumer.execute_target_person_consumer(
        env_file=env_file,
        dsn_env=dsn_env,
        reference_dsn_env=reference_dsn_env,
        item_code="I5B",
        execute=execute,
    )
    ...
    return {
        "generated_by": "scripts/dev/retrieval_v3_consumer.py",
        "command": "apply",
        "stage": "completion",
        "write_db": execute,
        "executed": execute,
...
def plan_consumer(
    *,
    ...
    completion = execute_completion_stage(env_file=env_file, dsn_env=dsn_env, reference_dsn_env=reference_dsn_env, execute=False)
    ...
    return {
        ...
        "missing_person_profiles": fetch_scalar(
            ...
            + """
              select count(*)::int as n
                ...
                left join retrieval_v3.person_profiles pp on pp.object_id = sp.id
               where pp.id is null
            """,
            params,
        ),
        "missing_talent_grade": fetch_scalar(
            ...
            + """
              select count(*)::int as n
                from scoped_persons sp
                join retrieval_v3.person_profiles pp on pp.object_id = sp.id
        ...
        "missing_person_roles": fetch_scalar(
            ...
            + """
              select count(*)::int as n
                from scoped_persons sp
               where not exists (
                    select 1 from retrieval_v3.person_roles pr
        ...
        "missing_person_affiliations": fetch_scalar(
            ...
            + """
              select count(*)::int as n
                from scoped_persons sp
               where not exists (
                    select 1 from retrieval_v3.person_affiliations pa
        ...
        "missing_script_variants": fetch_scalar(
            ...
            + """
              select count(*)::int as n
                from scoped_persons sp
                join retrieval_v3.objects o on o.id = sp.id
            ...
            select count(*)::int as n
             ...
             where not exists (
                   select 1
                     from retrieval_v3.target_objects tob
                     join retrieval_v3.objects o on o.id = tob.object_id
                     join retrieval_v3.person_roles pr on pr.object_id = o.id
                    where tob.target_id = st.id
                      and tob.object_role = 'target_emperor'
                      and o.object_type = 'person'
    ...
    if int(snapshot.get("missing_target_emperor_affiliations") or 0):
        warnings.append(
            reason_entry(
                ...
                message="目标皇帝缺朝代归属；不阻塞 retrieval_v3 消费，但会影响按朝代筛选和后续人物画像检索。",
            )
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Unified retrieval_v3 consumer entrypoint; default operations are dry-run/read-only.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--env-file", type=Path)
    common.add_argument("--dsn-env", default="EMPEROR_EVAL_RETRIEVAL_V3_DSN")
    common.add_argument("--reference-dsn-env", default="EMPEROR_EVAL_RETRIEVAL_V3_DSN")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "plan":
        ...
        payload = execute_completion_stage(env_file=args.env_file, dsn_env=args.dsn_env, reference_dsn_env=args.reference_dsn_env, execute=args.execute)
