...
from scripts.dev.retrieval_v3_factorization_worklists import scope_predicate  # noqa: E402
from scripts.dev.retrieval_v3_import_plan import json_param, stable_hash, write_json  # noqa: E402
...
from scripts.dev.retrieval_v3_judgment_worklists import agent_runtime_config, run_codex_tasks  # noqa: E402
from scripts.dev.retrieval_v3_material_review_tasks import PATCH_BEGIN, PATCH_END, extract_patch_rows  # noqa: E402
    ...
    cur.execute(
        ...
        artifact_agg as (
            select
              ...
              from retrieval_v3.source_pack_artifacts
        ...
        select
          ...
          join retrieval_v3.material_claims mc on mc.id = mrq.claim_id
         ...
         where mrq.review_kind = any(%s::text[])
           and mrq.queue_status = any(%s::retrieval_v3.rv3_queue_status[])
...
def candidate_payload_paths(row: Mapping[str, Any]) -> list[Path]:
    ...
    if target_code:
        run_root = ROOT / "tmp" / "retrieval_v3_clean_runs"
...
def prompt_for_task(*, task: Mapping[str, Any], workitems: Sequence[Mapping[str, Any]], patch_path: Path) -> str:
    return (
        "# retrieval_v3 claim/passage repair task\n\n"
...
def build_codex_tasks(workitems: Sequence[Mapping[str, Any]], *, output_root: Path, batch_size: int) -> list[dict[str, Any]]:
    ...
    for batch_index, batch in enumerate(chunks(workitems, batch_size), start=1):
        ...
        task = {
            ...
            "argv": agent_runtime_config.codex_task_argv("claim_passage_repair", exec_args=["-C", str(ROOT), "--dangerously-bypass-approvals-and-sandbox", "--output-last-message", str(last_message_path), "--json", "-"]),
        }
...
def write_worklist_outputs(*, output_root: Path, workitems: Sequence[Mapping[str, Any]], batch_size: int) -> dict[str, Any]:
    ...
    summary = {
        "generated_by": "scripts/dev/retrieval_v3_claim_passage_repair.py",
...
def fetch_document_id(cur: Any, *, source_pack_id: int, raw_document_code: str) -> int:
    cur.execute(
        """
        select id
          from retrieval_v3.source_documents
...
def upsert_repair_passage(cur: Any, *, context: Mapping[str, Any], candidate_slice: Mapping[str, Any], review_code: str) -> tuple[int, str]:
    ...
    payload = {
        "source": "retrieval_v3_claim_passage_repair",
        "review_code": review_code,
...
def replace_claim_passages(cur: Any, *, context: Mapping[str, Any], passage_ids: Sequence[int], passage_codes: Sequence[str], patch: Mapping[str, Any]) -> None:
    ...
    cur.execute("delete from retrieval_v3.claim_source_passages where claim_id = %s", (claim_id,))
    for passage_id in passage_ids:
        cur.execute(
            ...
            (
                ...
                json_param({"source": "retrieval_v3_claim_passage_repair", "patch": dict(patch)}),
            ),
        )
    ...
    cur.execute(
        """
        update retrieval_v3.material_claims
...
def update_review_queue(cur: Any, *, context: Mapping[str, Any], patch: Mapping[str, Any]) -> None:
    ...
    cur.execute(
        """
        update retrieval_v3.material_review_queue
           set queue_status = %s::retrieval_v3.rv3_queue_status,
               review_note = %s,
...
def close_control_plane_items(cur: Any, *, context: Mapping[str, Any], patch: Mapping[str, Any]) -> dict[str, int]:
    ...
    cur.execute(
        """
        update retrieval_v3.coverage_gap_events
           set status = 'resolved',
               event_payload = event_payload || %s::jsonb,
               updated_at = now()
         where event_payload->>'source' = 'retrieval_v3_claim_passage_audit'
    ...
    cur.execute(
        """
        update retrieval_v3.jobs
           set status = 'succeeded',
               payload = payload || %s::jsonb,
               last_error = '',
               updated_at = now()
         where kind = 'codex_material_review'
           and payload->'gap_event'->>'source' = 'retrieval_v3_claim_passage_audit'
    ...
    return {
        "retrieval_v3.coverage_gap_events": events,
        "retrieval_v3.jobs": jobs,
    }
        ...
        with conn.cursor() as cur:
            for patch in validated:
                ...
                if action in {"relink", "rewrite"}:
                    ...
                    counts["retrieval_v3.claim_source_passages"] += len(passage_ids)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Repair retrieval_v3 claim/passages mismatches from accepted clean packages.")
    ...
    worklist.add_argument("--batch-size", type=int, default=agent_runtime_config.resolve_agent_stage("claim_passage_repair")["batch_size"])
    ...
    run_plan.add_argument("--max-workers", type=int, default=agent_runtime_config.resolve_agent_stage("claim_passage_repair")["max_workers"])
    run_plan.add_argument("--timeout-seconds", type=int, default=agent_runtime_config.resolve_agent_stage("claim_passage_repair")["timeout_seconds"])
    ...
    apply = subparsers.add_parser("apply-patch", help="Apply a claim/passage repair JSONL patch to retrieval_v3.")
