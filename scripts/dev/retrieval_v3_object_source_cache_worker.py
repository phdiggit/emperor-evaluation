...
from scripts.dev import retrieval_v3_claim_cache as claim_cache  # noqa: E402
from scripts.dev import retrieval_v3_claim_extraction_worker as claim_worker  # noqa: E402
from scripts.dev import retrieval_v3_claim_plan_quality as claim_plan_quality  # noqa: E402
from scripts.dev import retrieval_v3_claim_quality as claim_quality  # noqa: E402
from scripts.dev import retrieval_v3_object_source_cache as object_cache  # noqa: E402
from scripts.dev.retrieval_v3_contracts import alias_script_variants, source_hints_from_source_targets, unique_strings  # noqa: E402
from scripts.dev.retrieval_v3_bootstrap import import_psycopg, load_env_file, resolve_dsn  # noqa: E402
from scripts.dev.retrieval_v3_pg_schema import DEFAULT_PG_SCHEMA, DEFAULT_V3_DSN_ENV, render_sql, schema_cursor  # noqa: E402


DEFAULT_DSN_ENV = DEFAULT_V3_DSN_ENV
DEFAULT_OUTPUT_ROOT = ROOT / "tmp" / "retrieval_v3_object_source_cache_runs"
DEFAULT_PAGE_CACHE_ROOT = ROOT / "tmp" / "retrieval_v3_source_pages"
DEFAULT_CLAIM_CACHE_ROOT = ROOT / "tmp" / "retrieval_v3_claim_cache"
...
def apply_schema(target_dsn: str, *, schema_name: str = DEFAULT_PG_SCHEMA) -> None:
    ...
    sql = (ROOT / "db" / "migrations" / "20260708_retrieval_v3_object_source_cache_jobs.sql").read_text(encoding="utf-8")
...
def upsert_job(cur: Any, job: Mapping[str, Any]) -> int:
    cur.execute(
        """
        insert into retrieval_v3.object_source_cache_jobs (
        ...
        on conflict (idem_key) do update set
            priority = least(retrieval_v3.object_source_cache_jobs.priority, excluded.priority),
            ...
            job_payload = retrieval_v3.object_source_cache_jobs.job_payload || excluded.job_payload,
            status = case
                when retrieval_v3.object_source_cache_jobs.status::text in ('succeeded', 'running', 'cancelled')
                    then retrieval_v3.object_source_cache_jobs.status
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as raw_cur:
            ...
            cur.execute(
                ...
                update retrieval_v3.object_source_cache_jobs j
                   set status = 'running',
...
def create_job_run(cur: Any, *, job: Mapping[str, Any], worker_id: str, run_code: str, input_fingerprint: str) -> int:
    cur.execute(
        """
        insert into retrieval_v3.object_source_cache_job_runs (
    ...
    *,
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as raw_cur:
            ...
            cur.execute(
                """
                update retrieval_v3.object_source_cache_job_runs
...
def finish_job_run(
    ...
    *,
    ...
    if status == "succeeded":
        cur.execute(
            """
            update retrieval_v3.object_source_cache_jobs
        ...
        cur.execute(
            ...
            update retrieval_v3.object_source_cache_jobs
               set status = (
                   ...
                   )::retrieval_v3.rv3_object_source_cache_job_status,
                   locked_by = null,
                   locked_at = null,
                   lease_until = null,
                   last_error = %s,
...
def selected_object_cache_slices(
    ...
    *,
    ...
    for person_name in sorted(by_person):
        ...
        anchors = [
            row for row in rows_for_person
            if text((row.get("object_source_cache") or {}).get("slice_kind")) == "summary_lead_term_anchor"
        ]
        forced: list[dict[str, Any]] = []
        for family_terms in (
            set(object_cache.CAREER_DISCOVERY_TERMS),
            set(object_cache.SUMMARY_LEAD_HIGH_PRIORITY_TERMS) | set(object_cache.SUMMARY_LEAD_LOW_PRIORITY_TERMS),
        ):
            anchor = next((row for row in anchors if family_terms.intersection(row.get("lead_terms") or [])), None)
            if anchor is not None and anchor not in forced:
                forced.append(anchor)
        for index, anchor in enumerate(forced[:per_person_limit], start=1):
            if anchor not in retained:
                retained[-index] = anchor
...
def object_cache_to_claim_candidates(
    *,
    ...
    return {
        "schema_version": 1,
        "generated_by": "scripts/dev/retrieval_v3_object_source_cache_worker.py claim-plan",
        "task_identity": {
            "emperor_name": emperor_name,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PostgreSQL-backed worker for retrieval_v3 object source cache jobs.")
    ...
    once_cmd.add_argument("--worker-id", default="retrieval_v3_object_source_cache_worker")
