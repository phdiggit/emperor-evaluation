...
from scripts.dev import retrieval_v3_candidate_prompt as candidate_prompt  # noqa: E402
from scripts.dev import retrieval_v3_claim_cache as fs_cache  # noqa: E402
from scripts.dev import retrieval_v3_claim_cache_pg as pg_cache  # noqa: E402
from scripts.dev import retrieval_v3_clean_runner as clean_runner  # noqa: E402
from scripts.dev.retrieval_v3_bootstrap import import_psycopg, load_env_file, resolve_dsn  # noqa: E402
from scripts.dev.retrieval_v3_pg_schema import (  # noqa: E402
    DEFAULT_PG_SCHEMA,
    DEFAULT_V3_DSN_ENV,
    render_sql,
    schema_cursor,
...
DEFAULT_RUN_ROOT = ROOT / "tmp" / "retrieval_v3_claim_extraction_runs"
...
def apply_schema(target_dsn: str, *, schema_name: str = DEFAULT_PG_SCHEMA) -> None:
    ...
    sql = (ROOT / "db" / "migrations" / "20260708_retrieval_v3_claim_extraction_jobs.sql").read_text(encoding="utf-8")
...
def upsert_job(cur: Any, job: Mapping[str, Any]) -> int:
    cur.execute(
        """
        insert into retrieval_v3.claim_extraction_jobs (
        ...
        on conflict (idem_key) do update set
            priority = least(retrieval_v3.claim_extraction_jobs.priority, excluded.priority),
            ...
            job_payload = retrieval_v3.claim_extraction_jobs.job_payload || excluded.job_payload,
            status = case
                when retrieval_v3.claim_extraction_jobs.status::text in ('succeeded', 'running', 'cancelled')
                    then retrieval_v3.claim_extraction_jobs.status
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as raw_cur:
            ...
            cur.execute(
                ...
                update retrieval_v3.claim_extraction_jobs j
                   set status = 'running',
...
def create_job_run(cur: Any, *, job: Mapping[str, Any], worker_id: str, run_code: str, input_fingerprint: str) -> int:
    cur.execute(
        """
        insert into retrieval_v3.claim_extraction_job_runs (
...
def finish_job_run(
    ...
    *,
    ...
    cur.execute(
        """
        update retrieval_v3.claim_extraction_job_runs
           set status = %s,
               ended_at = now(),
    ...
    if status == "succeeded":
        cur.execute(
            """
            update retrieval_v3.claim_extraction_jobs
        ...
        cur.execute(
            ...
            update retrieval_v3.claim_extraction_jobs
               set status = (
                   ...
                   )::retrieval_v3.rv3_claim_extraction_job_status,
                   locked_by = null,
                   locked_at = null,
                   lease_until = null,
                   last_error = %s,
...
def write_mini_run_artifacts(
    *,
    ...
    summary = {
        "ok": True,
        "generated_by": "scripts/dev/retrieval_v3_claim_extraction_worker.py",
        "run_root": str(run_root),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Claim-only extraction worker for uncovered retrieval_v3 candidate slices.")
    ...
    plan.add_argument("--worker-id", default="retrieval_v3_claim_extraction_worker")
    ...
    once_cmd.add_argument("--worker-id", default="retrieval_v3_claim_extraction_worker")
