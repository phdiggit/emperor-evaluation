...
- `retrieval_v3_coverage_controller.py` 是 v3 native claim cache 到 factorization 的只读覆盖控制器，按对象核对 source slice、claim evidence、同 event group 事实链、material、candidate、binding 和 factor judgment，并生成确定性修复动作；它不写队列、不改数据库、不调用模型，也不得把“已观察管线闭合”解释为历史事件覆盖完整。未提供预期史源/事件清单时，房玄龄、李靖等对象仍需上游 source/event inventory 审计。
- `retrieval_v3_expected_event_inventory.py` 按对象生成有限的重大任用/职责/结果事件检索线索 task，并验收 agent patch；未指定对象或 `--coverage-status` 时默认只排查管线 `complete` 的假绿对象，每对象必须独立输出一行，默认每 task 合批 2 对象以控制重复上下文成本，结果固定为 `retrieval_lead_only`、`scoring_allowed=false`，只能交给 coverage controller 对账或触发回源，不能直接生成 claim、binding 或分数。
- `retrieval_v3_expected_event_reconciliation.py` 把 expected event 与当前最相近的 event groups、claims 和缓存 source slices 一并交给有限语义复核，输出 `already_covered` / `rebuild_event_group` / `reextract_cached_source` / `fetch_missing_source` / review 类结论；首轮默认按 `initial_actionability` 计算现有材料可行动率，低于 50% 时 `progress_allowed=false`，必须停止并优化，禁止直接消费此前字符串匹配生成的回源清单。补源试验可用 `--source-cache-root` 只读叠加新 object-source-cache 切片，并用 `--object-name` 做单对象复验；重抽后可用 `--claim-result-json` 叠加本地抽取 claim，并用 `--event-selection-jsonl` 只复验明确修复事件，合并时必须传 `--gate-mode repair_verification`，重复 `reextract_cached_source` 不得冒充已经修复。
- `retrieval_v3_expected_event_repair_plan.py` 只在 reconciliation `progress_allowed=true` 后消费 `reextract_cached_source` / `rebuild_event_group`，把明确引用的缓存切片裁成窄 reextract cache，并生成事件组重建审查清单；它不调用智能体、不抓新源、不写库、不计分，未被 reconciliation 引用的切片不得夹带进入 claim-plan。
- `retrieval_v3_expected_event_source_refinement.py` 只消费 reconciliation 的 `fetch_missing_source` 结论，把逐事件回源请求按皇帝和对象合并为对象级补源包及兼容 object-source-cache 的 seed，并输出仅含本轮补源事件的 `event_selection.jsonl` 供定向复验；它机械合并别名、史源线索、查询词和已检查引用，不调用智能体、不投递 job、不写库、不计分，也不得绕过 reconciliation 硬门。
...
- 所有 Codex 环节的模型、reasoning、batch/shard、并发和 timeout 统一从 `data/configs/project_config.yml` 的 `tooling.agent_runtime` 读取；公共解析只用 `scripts/shared/agent_runtime_config.py`，不得在新任务生成器、worker 或服务启动脚本中另写模型默认值。
- `retrieval_v3_runtime_release.py` 只从干净工作区的已提交 SHA 打包运行代码与项目配置；服务器端默认只读验收，显式 `apply --execute` 才允许原子切换 `current` 并重启传入的 systemd unit。不得用该工具顺带执行数据库迁移、消费或计分。
