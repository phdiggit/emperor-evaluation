...
- `retrieval_v3_expected_event_reconciliation.py` 把 expected event 与当前最相近的 event groups、claims 和缓存 source slices 一并交给有限语义复核，输出 `already_covered` / `rebuild_event_group` / `reextract_cached_source` / `fetch_missing_source` / review 类结论；默认现有材料解决率低于 50% 时 `progress_allowed=false`，必须停止并优化，禁止直接消费此前字符串匹配生成的回源清单。补源试验可用 `--source-cache-root` 只读叠加新 object-source-cache 切片，并用 `--object-name` 做单对象复验。
- `retrieval_v3_expected_event_source_refinement.py` 只消费 reconciliation 的 `fetch_missing_source` 结论，把逐事件回源请求按皇帝和对象合并为对象级补源包及兼容 object-source-cache 的 seed；它机械合并别名、史源线索、查询词和已检查引用，不调用智能体、不投递 job、不写库、不计分，也不得绕过 reconciliation 硬门。
