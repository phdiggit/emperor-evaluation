...
SOURCE = "retrieval_v2_object_source_cache_pg_backfill"
...
def upsert_object(cur: Any, row: Mapping[str, Any]) -> int:
    cur.execute(
        ...
        on conflict on constraint rv2_objects_identity_key_uk do update set
            ...
            identity_payload = retrieval_v2.objects.identity_payload || excluded.identity_payload,
            updated_at = now()
        returning id
        """,
        (
...
def upsert_object_name(cur: Any, row: Mapping[str, Any], *, object_id: int) -> int:
    cur.execute(
        ...
        on conflict on constraint rv2_object_names_name_uk do update set
            ...
            name_payload = retrieval_v2.object_names.name_payload || excluded.name_payload
        returning id
        """,
        (
...
def upsert_profile(cur: Any, row: Mapping[str, Any], *, object_id: int) -> int:
    cur.execute(
        ...
        on conflict on constraint rv2_person_profiles_object_uk do update set
            ...
            review_status = case
                when retrieval_v2.person_profiles.review_status in ('rejected', 'retired') then retrieval_v2.person_profiles.review_status
                else excluded.review_status
            end,
...
def upsert_affiliation(cur: Any, row: Mapping[str, Any], *, object_id: int) -> int:
    cur.execute(
        ...
        on conflict on constraint rv2_person_affiliations_key_uk do update set
            ...
            review_status = case
                when retrieval_v2.person_affiliations.review_status in ('rejected', 'retired') then retrieval_v2.person_affiliations.review_status
                else excluded.review_status
            end,
            affiliation_payload = retrieval_v2.person_affiliations.affiliation_payload || excluded.affiliation_payload,
            updated_at = now()
        returning id
        """,
        (
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# retrieval_v2 object source cache PG backfill report",
        "",
