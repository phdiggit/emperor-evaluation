...
class RetrievalV3BatchTaskgenError(RuntimeError):
    pass
...
def _shared_contract(contexts: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    if not contexts:
        raise RetrievalV3BatchTaskgenError("batch taskgen requires at least one context")
    first = contexts[0]
    return {
...
def build_batch_discovery_prompt(
    ...
    *,
    ...
    return (
        "你是 emperor-evaluation retrieval_v3 clean batch taskgen discovery worker。\n"
...
def target_discoveries_from_payload(
    ...
    *,
    ...
    if not isinstance(rows, list):
        raise RetrievalV3BatchTaskgenError("batch taskgen output must contain a targets list")
    ...
    if missing:
        raise RetrievalV3BatchTaskgenError(f"batch taskgen missing targets: {missing}")
    return result
