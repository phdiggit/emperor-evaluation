...
from scripts.dev.retrieval_v3_import_plan import claim_passage_alignment_issue, stable_hash  # noqa: E402
    ...
    *,
    ...
    rule_filter = """
           and (
                %s = ''
                or exists (
                    select 1 from retrieval_v3.claim_rule_bindings b
                     where b.claim_id = mc.id and b.rule_code = %s
                )
                or exists (
                    select 1 from retrieval_v3.claim_rule_binding_candidates c
    ...
    cur.execute(
        ...
        select
            ...
            (
                select crr.id
                  from retrieval_v3.rule_contract_rules crr
            ...
            (
                select count(*) from retrieval_v3.claim_rule_binding_candidates c
                 where c.claim_id = mc.id
            ...
            (
                select count(*) from retrieval_v3.claim_rule_bindings b
                 where b.claim_id = mc.id
            )::int as binding_count,
            (
                select count(*) from retrieval_v3.claim_rule_binding_factor_judgments j
                 where j.claim_id = mc.id and j.review_status = 'accepted'
            )::int as factor_judgment_count,
            (
                select count(*) from retrieval_v3.claim_rule_binding_material_scores ms
                 where ms.claim_id = mc.id
            )::int as material_score_count,
            (
                select count(*) from retrieval_v3.material_review_queue mrq
...
def build_audit(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "generated_by": "scripts/dev/retrieval_v3_claim_passage_audit.py",
        "write_db": False,
        "executed": False,
        "totals": {
            "claims": len(claims),
...
def enqueue_material_reviews(cur: Any, flagged_claims: Sequence[Mapping[str, Any]]) -> dict[str, int]:
    ...
    for row in flagged_claims:
        ...
        payload = {
            "source": "retrieval_v3_claim_passage_audit",
        ...
        cur.execute(
            ...
            on conflict on constraint rv3_material_review_queue_idem_uk do update set
                ...
                queue_status = retrieval_v3.material_review_queue.queue_status,
                priority = least(retrieval_v3.material_review_queue.priority, excluded.priority),
                diagnosis = excluded.diagnosis,
                recommended_action = excluded.recommended_action,
                review_payload = retrieval_v3.material_review_queue.review_payload || excluded.review_payload,
                updated_at = now()
            """,
            (
                review_code,
    ...
    for row in flagged_claims:
        ...
        cur.execute(
            ...
            on conflict on constraint rv3_coverage_gap_events_idem_uk do update set
                ...
                priority = least(retrieval_v3.coverage_gap_events.priority, excluded.priority),
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v3 claim passage audit",
        "",
        f"- ok: `{payload.get('ok')}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Audit accepted retrieval_v3 claims for claim_summary/source_passage drift.")
