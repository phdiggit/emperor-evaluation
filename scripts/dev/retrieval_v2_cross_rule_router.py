...
def candidate_row(row: Mapping[str, Any], route: RouteSpec) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "created_from": "retrieval_v2_cross_rule_router",
        "route_status": hint_status,
        "candidate_lane": candidate_lane,
        "hint_status": hint_status,
        "required_facts_present": required_facts_present,
        "routed_by_profile": "retrieval_v2_cross_rule_router",
...
def fetch_source_rows(cur: Any, *, item_code: str, source_rule_code: str, emperors: Sequence[str]) -> list[dict[str, Any]]:
    ...
    cur.execute(
        ...
        select
          ...
          join retrieval_v2.source_packs sp on sp.target_id = t.id
          join retrieval_v2.material_claims mc on mc.source_pack_id = sp.id
          join retrieval_v2.claim_rule_bindings crb on crb.claim_id = mc.id
...
def build_plan(
    ...
    *,
    ...
    return {
        "ok": True,
        "generated_by": "scripts/dev/retrieval_v2_cross_rule_router.py",
        "item_code": item_code,
...
def upsert_candidate(cur: Any, row: Mapping[str, Any]) -> int | None:
    cur.execute(
        ...
        on conflict on constraint rv2_claim_rule_binding_candidates_uk do update set
            candidate_contract_rule_id = coalesce(retrieval_v2.claim_rule_binding_candidates.candidate_contract_rule_id, excluded.candidate_contract_rule_id),
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# retrieval_v2 cross-rule router",
        "",
