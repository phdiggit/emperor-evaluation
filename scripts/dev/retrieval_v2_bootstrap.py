...
class RetrievalV2BootstrapError(RuntimeError):
    pass
...
def resolve_dsn(env_name: str) -> str:
    ...
    if not value:
        raise RetrievalV2BootstrapError(f"missing PostgreSQL DSN env var: {env_name}")
    return value


def load_env_file(path: Path) -> list[str]:
    if not path.exists():
        raise RetrievalV2BootstrapError(f"env file missing: {path}")
    loaded: list[str] = []
...
def import_psycopg():
    try:
        ...
        raise RetrievalV2BootstrapError("psycopg is required for live retrieval_v2 bootstrap") from exc
...
def fetch_json_rows(cur: Any, sql: str, params: Sequence[Any]) -> list[dict[str, Any]]:
    ...
    for row in cur.fetchall():
        ...
        if not isinstance(payload, dict):
            raise RetrievalV2BootstrapError("expected PostgreSQL to_jsonb(row) payload to decode as object")
        rows.append(payload)
    return rows
    ...
    with psycopg.connect(source_dsn, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            if not table_exists(cur, "eval_items") or not table_exists(cur, "eval_rules"):
                raise RetrievalV2BootstrapError("source database must contain public.eval_items and public.eval_rules")
            ...
            if not item_rows:
                raise RetrievalV2BootstrapError(f"source database has no eval_items row for item_code={item_code}")
...
def upsert_item(cur: Any, row: Mapping[str, Any]) -> int:
    ...
    if not item_code:
        raise RetrievalV2BootstrapError(f"eval_items source row lacks item_code: {row}")
    cur.execute(
        """
        insert into retrieval_v2.eval_items (
...
def upsert_rule(cur: Any, row: Mapping[str, Any], item_ids: Mapping[int, int]) -> int:
    ...
    if not rule_code:
        raise RetrievalV2BootstrapError(f"eval_rules source row lacks rule_code: {row}")
    cur.execute(
        """
...
def upsert_predicate_option(cur: Any, row: Mapping[str, Any], item_ids_by_code: Mapping[str, int], rule_ids_by_code: Mapping[str, int]) -> int:
    ...
    cur.execute(
        """
        insert into retrieval_v2.fact_relation_predicate_options (
...
def upsert_contract(cur: Any, *, snapshot: SourceSnapshot, item_code: str, contract_code: str, source_database_label: str) -> int:
    payload = {
        ...
        "tool": "scripts/dev/retrieval_v2_bootstrap.py",
    }
    cur.execute(
        """
        insert into retrieval_v2.rule_contracts (
...
def seed_target(cur: Any, *, emperor_name: str, item_code: str, contract_id: int, contract_code: str) -> dict[str, Any]:
    ...
    payload = {"seed_source": "retrieval_v2_bootstrap", "contract_code": contract_code}
    cur.execute(
        """
        insert into retrieval_v2.retrieval_targets (
        ...
        on conflict (contract_id, emperor_name) do update set
            ...
            item_code = excluded.item_code,
            target_payload = retrieval_v2.retrieval_targets.target_payload || excluded.target_payload,
            updated_at = now()
    ...
    cur.execute(
        """
        insert into retrieval_v2.target_aliases (target_id, alias, alias_type, norm_alias, source)
    ...
    cur.execute(
        ...
        select id, rule_code, rule_order, is_core_for_retrieval, requirement_payload
          from retrieval_v2.rule_contract_rules
         where contract_id = %s
    ...
    for rule in cur.fetchall():
        ...
        cur.execute(
            """
            insert into retrieval_v2.target_rule_requirements (
        ...
        cur.execute(
            """
            insert into retrieval_v2.retrieval_intents (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not report["actions"]:
        raise RetrievalV2BootstrapError("no action requested; use --print-schema, --apply-schema, or --copy-rule-contract")
...
if __name__ == "__main__":  # pragma: no cover
    ...
    except RetrievalV2BootstrapError as exc:
