...
REGISTRY_PATH = "docs/治理规则/docs_registry.json"
...
def check_registry(registry_path: str = REGISTRY_PATH) -> list[str]:
    ...
    for doc in documents:
        ...
        if placement_action == "keep_governance_exception" and not path.startswith("docs/治理规则/"):
            allowed_governance_paths = set(registry.get("governance_exception_paths", []))
            if path not in allowed_governance_paths:
                problems.append(
                    f"{path}: placement_action=keep_governance_exception is only allowed under docs/治理规则/ or governance_exception_paths"
                )
