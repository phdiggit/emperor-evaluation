...
from scripts.dev import retrieval_v3_discovery_profiles as discovery_profiles
from scripts.dev import retrieval_v3_batch_taskgen as batch_taskgen
from scripts.dev import retrieval_v3_contracts as contracts
from scripts.dev import retrieval_v3_runtime_paths as runtime_paths
...
from scripts.dev import retrieval_v3_taskgen_preseed as taskgen_preseed
from scripts.dev import retrieval_v3_clean_runner as runner
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run retrieval_v3 clean taskgen/candidate/alias/judge pipeline.")
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Reusable retrieval_v3 discovery profile JSON; repeatable. Matching profiles skip Codex taskgen.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="EMPEROR_EVAL_RETRIEVAL_V3_DSN",
...
def _apply_claim_cache_stable_rerun_preset(args: argparse.Namespace) -> None:
    ...
    if args.claim_cache_root is None:
        raise runner.RetrievalV3CleanRunnerError("--claim-cache-stable-rerun-preset requires --claim-cache-root")
...
def _expand_object_sources_after_taskgen(
    *,
    ...
    if issues:
        raise runner.RetrievalV3CleanRunnerError(f"object source presearch produced invalid task: {issues}")
...
def _run_batch_taskgen(
    *,
    ...
    for index, name in enumerate(names):
        ...
        if issues:
            raise runner.RetrievalV3CleanRunnerError(f"batch taskgen discovery produced invalid task: {issues}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        raise runner.RetrievalV3CleanRunnerError(
    ...
    if args.task:
        ...
        raise runner.RetrievalV3CleanRunnerError("provide --task or at least one --emperor")
