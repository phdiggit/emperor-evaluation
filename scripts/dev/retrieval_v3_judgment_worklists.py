    ...
    *,
    ...
    for source in sources:
        ...
        if referenced_claim_key:
            ...
            raw_referenced = cur.fetchone()
            if not raw_referenced:
                raise JudgmentWorklistError(
                    f"authority source {referenced_claim_key} is not active for object_id={object_id}"
                )
            referenced = dict(raw_referenced)
