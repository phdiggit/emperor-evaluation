...
select
    ...
    q.queue_payload
from retrieval_v3.object_resolution_queue q
join retrieval_v3.retrieval_targets rt on rt.id = q.target_id
left join retrieval_v3.source_packs sp on sp.id = q.source_pack_id
...
def build_object_plan(queue_rows: Sequence[Mapping[str, Any]], link_rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    operation_counts = {
        "retrieval_v3.objects": sum(1 for row in accepted.values() if text(row.get("queue_status")) == "ready"),
        "retrieval_v3.object_names": object_name_count,
        "retrieval_v3.target_objects": len(accepted),
        "retrieval_v3.object_resolution_queue": sum(1 for row in accepted.values() if text(row.get("queue_status")) == "ready"),
        "retrieval_v3.material_object_links": len(link_candidates),
    }
    return {
        "generated_by": "scripts/dev/retrieval_v3_object_consumer.py",
...
def upsert_object(cur: Any, row: Mapping[str, Any]) -> int:
    ...
    payload = {
        "source": "retrieval_v3_object_consumer",
    ...
    cur.execute(
        ...
        values (%s, %s, %s, %s, %s::retrieval_v3.rv3_object_type, 'active', %s, %s::jsonb)
...
def upsert_object_name(cur: Any, row: Mapping[str, Any], object_id: int, *, name_kind: str = "canonical", name_text: str | None = None) -> int:
    ...
    cur.execute(
        ...
        values (%s, %s, %s, %s, %s::retrieval_v3.rv3_object_name_kind, %s, 'retrieval_v3_object_consumer', 'accepted', %s::jsonb)
...
def upsert_target_object(cur: Any, row: Mapping[str, Any], object_id: int, first_claim_id: int | None) -> int:
    identity_key = object_identity_key(row)
    payload = {"source": "retrieval_v3_object_consumer", "resolution_code": text(row.get("resolution_code"))}
    cur.execute(
        """
...
def mark_queue_resolved(cur: Any, row: Mapping[str, Any], object_id: int) -> int:
    cur.execute(
        """
        update retrieval_v3.object_resolution_queue
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v3 object consumer report",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Consume retrieval_v3 object resolution queue; default is DB-backed dry-run.")
