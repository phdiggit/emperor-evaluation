...
from scripts.dev.retrieval_v3_target_alias_backfill import (  # noqa: E402
    DEFAULT_ALIAS_FILE,
...
def fetch_owner_alias_book(cur: Any) -> OwnerAliasBook:
    cur.execute(
        ...
        select
          ...
          join retrieval_v3.target_aliases a on a.target_id = t.id
    ...
    *,
    ...
    cur.execute(
        ...
        select
          ...
          from retrieval_v3.claim_atomic_facts f
          left join (
                select
                    e.claim_key,
                    string_agg(
...
def apply_rebind_plan(cur: Any, rows: Sequence[Mapping[str, Any]]) -> int:
    ...
    for row in rows:
        payload = {
            ...
            "actor": text(row.get("actor")),
            "source": "retrieval_v3_claim_owner_audit",
        }
        cur.execute(
            """
...
def apply_review_status_plan(cur: Any, rows: Sequence[Mapping[str, Any]]) -> int:
    ...
    for row in rows:
        ...
        cur.execute(
            """
            update retrieval_v3.claim_cache
               set status = 'needs_review'::retrieval_v3.rv3_claim_cache_status,
                   fact_payload = fact_payload || %s::jsonb,
                   updated_at = now()
             where claim_key = %s
               and emperor_name = %s
...
def audit_claim_owners(
    *,
    ...
    return {
        "ok": True,
        "generated_by": "scripts/dev/retrieval_v3_claim_owner_audit.py",
