...
def sync_pg_deletes(*, dsn: str, dropped_evidence_keys: Sequence[str], execute: bool, schema_name: str) -> dict[str, Any]:
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as raw_cur:
            ...
            cur.execute(
                "select evidence_key, claim_key, source_slice_ref from retrieval_v3.claim_evidence where evidence_key = any(%s)",
            ...
            if execute:
                cur.execute(
                    "delete from retrieval_v3.claim_evidence where evidence_key = any(%s) returning evidence_key",
                    (list(dropped_evidence_keys),),
                )
...
def apply_review_patch(
    *,
    ...
    report: dict[str, Any] = {
        ...
        "generated_by": "scripts/dev/retrieval_v3_claim_cache_review_patch.py",
        "patch_code": patch_code,
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v3 claim cache review patch",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Apply reviewed JSONL patches to a filesystem retrieval_v3 claim cache; dry-run by default.")
