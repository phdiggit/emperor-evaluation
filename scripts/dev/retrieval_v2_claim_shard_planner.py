...
def ordered_rows(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return sorted(
        ...
        key=lambda row: (
            CLASS_RANK.get(text(row.get("owner_anchor_class")), 9),
    ...
    *,
    ...
    for row in ordered_rows(rows):
        ...
        if not must_start_new and current is not None:
            ...
            exceeds_slices = max_slices_per_shard > 0 and len(current["slice_codes"]) >= max_slices_per_shard
            exceeds_chars = max_chars_per_shard > 0 and current["estimated_slice_chars"] + row_cost > max_chars_per_shard
        ...
        current["slice_codes"].append(text(row.get("slice_code")))
        current["estimated_slice_chars"] += row_cost
        current["event_signal_score"] += int(row.get("event_signal_score") or 0)
    return shards
...
def apply_owner_aware_shard_plan(
    ...
    *,
    ...
    retained = [row for row in classified if row.get("owner_anchor_class") != "D"]
    excluded = [
        {
