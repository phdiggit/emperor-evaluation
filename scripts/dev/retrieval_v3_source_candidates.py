...
from scripts.dev.retrieval_v3_source_document_policy import source_document_skip
...
class RetrievalV3CandidateError(RuntimeError):
    pass
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RetrievalV3CandidateError(f"expected object JSON: {path}")
    return payload
...
def fetch_wikisource_title(title: str, *, timeout: int) -> str:
    ...
    if not isinstance(html_text, str) or not html_text:
        raise RetrievalV3CandidateError(f"empty Wikisource page: {title}")
...
def fetch_document_text(
    ...
    *,
    ...
    if title:
        ...
        except RetrievalV3CandidateError:
            if not url:
                raise
        ...
        raise RetrievalV3CandidateError("document requires text, wikisource_title/title, or url")
...
def build_candidates(
    ...
    *,
    ...
    if not isinstance(source_documents, list) or not source_documents:
        raise RetrievalV3CandidateError("task requires source_documents")
    ...
    payload = {
        "generated_by": "scripts/dev/retrieval_v3_source_candidates.py",
        "schema_version": 1,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build retrieval_v3 source candidate slices for Codex CLI judging.")
...
if __name__ == "__main__":
    ...
    except RetrievalV3CandidateError as exc:
