...
from scripts.dev.retrieval_v3_import_plan import (  # noqa: E402
    ImportPlanError,
    build_plan,
...
def upsert_source_pack(cur: Any, row: Mapping[str, Any], lookup: Mapping[str, Any], *, source_pack_status: str = "accepted") -> int:
    ...
    cur.execute(
        """
        insert into retrieval_v3.source_packs (
...
def upsert_source_pack_artifact(cur: Any, row: Mapping[str, Any], pack_ids: Mapping[str, int]) -> int:
    ...
    cur.execute(
        """
        insert into retrieval_v3.source_pack_artifacts (
            source_pack_id, artifact_kind, artifact_path, sha256, artifact_payload
        )
        values (%s, %s, %s, %s, %s::jsonb)
        on conflict on constraint rv3_source_pack_artifacts_uk do update set
...
def upsert_source_document(cur: Any, row: Mapping[str, Any], pack_ids: Mapping[str, int]) -> int:
    ...
    cur.execute(
        """
        insert into retrieval_v3.source_documents (
...
def upsert_source_passage(cur: Any, row: Mapping[str, Any], document_ids: Mapping[str, int]) -> int:
    cur.execute(
        """
        insert into retrieval_v3.source_passages (
...
def upsert_material_claim(cur: Any, row: Mapping[str, Any], pack_ids: Mapping[str, int], passage_ids: Mapping[str, int]) -> int:
    ...
    cur.execute(
        ...
        on conflict (source_pack_id, raw_claim_code) where btrim(raw_claim_code) <> '' do update set
            ...
            review_status = case
                when retrieval_v3.material_claims.review_status = 'pending' then excluded.review_status
...
def upsert_claim_source_passage(cur: Any, *, claim_id: int, passage_id: int, pack_id: int, relation_payload: Mapping[str, Any]) -> int:
    cur.execute(
        """
        insert into retrieval_v3.claim_source_passages (
            claim_id, source_passage_id, source_pack_id, relation_kind, relation_payload
        )
        values (%s, %s, %s, 'supporting_quote', %s::jsonb)
        on conflict on constraint rv3_claim_source_passages_uk do update set
    ...
    *,
    ...
    cur.execute(
        ...
        on conflict on constraint rv3_claim_rule_bindings_uk do update set
            ...
            confidence = excluded.confidence,
            review_status = case
                when retrieval_v3.claim_rule_bindings.review_status = 'pending' then excluded.review_status
...
def upsert_claim_rule_binding_candidate(
    ...
    *,
    ...
    cur.execute(
        ...
        values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s::jsonb, %s, %s, %s, %s::retrieval_v3.rv3_claim_direction, %s, %s, %s, 'pending', %s, %s::jsonb)
        on conflict on constraint rv3_claim_rule_binding_candidates_code_uk do update set
            ...
            resolved_binding_id = coalesce(retrieval_v3.claim_rule_binding_candidates.resolved_binding_id, excluded.resolved_binding_id),
...
def upsert_coverage_gap_event(
    ...
    *,
    ...
    cur.execute(
        """
        insert into retrieval_v3.coverage_gap_events (
        ...
        on conflict on constraint rv3_coverage_gap_events_idem_uk do update set
            ...
            status = case
                when retrieval_v3.coverage_gap_events.status in ('queued', 'running', 'retry_wait', 'deferred', 'resolved', 'blocked', 'cancelled')
                    then retrieval_v3.coverage_gap_events.status
...
def upsert_object_resolution_queue(
    ...
    *,
    ...
    cur.execute(
        """
        insert into retrieval_v3.object_resolution_queue (
        ...
        values (%s, %s, %s, %s, null, %s, %s, %s::retrieval_v3.rv3_object_type, %s, %s, %s::retrieval_v3.rv3_queue_status, 100, %s, '', null, %s::jsonb)
        on conflict on constraint rv3_object_resolution_queue_idem_uk do update set
            ...
            queue_status = case
                when retrieval_v3.object_resolution_queue.queue_status in ('running', 'resolved', 'blocked', 'cancelled')
                    then retrieval_v3.object_resolution_queue.queue_status
                else excluded.queue_status
            end,
            diagnosis = excluded.diagnosis,
            resolution_note = case
                when btrim(retrieval_v3.object_resolution_queue.resolution_note) <> '' then retrieval_v3.object_resolution_queue.resolution_note
...
def upsert_material_review_queue(cur: Any, row: Mapping[str, Any], *, claim_ids: Mapping[str, int], binding_ids: Mapping[str, int]) -> int:
    ...
    cur.execute(
        """
        insert into retrieval_v3.material_review_queue (
        ...
        on conflict on constraint rv3_material_review_queue_idem_uk do update set
            review_code = excluded.review_code,
            binding_id = excluded.binding_id,
            review_kind = excluded.review_kind,
            queue_status = case
                when retrieval_v3.material_review_queue.queue_status in ('running', 'resolved', 'blocked', 'cancelled')
                    then retrieval_v3.material_review_queue.queue_status
                else excluded.queue_status
            end,
            diagnosis = excluded.diagnosis,
            recommended_action = excluded.recommended_action,
            review_note = case
                when btrim(retrieval_v3.material_review_queue.review_note) <> '' then retrieval_v3.material_review_queue.review_note
...
def execute_upserts(
    ...
    *,
    ...
    for row in rows["source_pack_artifacts"]:
        ...
        counts["retrieval_v3.source_pack_artifacts"] += 1
    ...
    for row in rows["coverage_gap_events"]:
        ...
        counts["retrieval_v3.coverage_gap_events"] += 1
    ...
    for row in review_rows["object_resolution_worklist"]:
        ...
        counts["retrieval_v3.object_resolution_queue"] += 1
    ...
    for row in review_rows["material_review_worklist"]:
        ...
        counts["retrieval_v3.material_review_queue"] += 1
        ...
        with conn.cursor() as raw_cur:
            ...
            report = {
                **{key: value for key, value in plan.items() if key != "operations"},
                "generated_by": "scripts/dev/retrieval_v3_import_executor.py",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Apply retrieval_v3 normalized rows; default is a DB-backed dry-run.")
    ...
    apply.add_argument("--pg-schema", default="retrieval_v3", help="Schema used by the importer; defaults to retrieval_v3 for compatibility.")
    apply.add_argument("--source-pack-status", choices=["accepted", "draft", "ready", "needs_refinement"], default="accepted")
    apply.add_argument("--execute", action="store_true", help="Actually write retrieval_v3 consumption rows. Omit for dry-run.")
    return parser
