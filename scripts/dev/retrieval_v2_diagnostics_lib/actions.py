...
from scripts.dev.retrieval_v2_diagnostics_lib.common import DEFAULT_RULE_CODE, text

def next_command_for(code: str, *, item_code: str, rule_code: str, formula_code: str, scope: str) -> str:
    output_root = f"tmp/retrieval_v2_consumption/diagnostics_next_{item_code}_{rule_code or 'all'}"
    if code == "material_review_pending":
        return (
            "python scripts/dev/retrieval_v2_material_review_consumer.py worklist "
    ...
    if code == "factorization_required":
        return (
            "python scripts/dev/retrieval_v2_factorization_worklists.py worklist "
    ...
    if code in {"material_score_required", "rule_score_required", "rule_score_stale"}:
        return (
            "python scripts/dev/retrieval_v2_rule_scorer.py apply "
            f"--env-file .env --item-code {item_code} --rule-code {rule_code or DEFAULT_RULE_CODE} "
    ...
    if code == "role_matched_object_link_missing":
        return (
            "python scripts/dev/retrieval_v2_object_consumer.py apply "
