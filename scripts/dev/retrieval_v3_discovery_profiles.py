...
class RetrievalV3DiscoveryProfileError(RuntimeError):
    pass
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RetrievalV3DiscoveryProfileError(f"expected object JSON: {path}")
    return payload
...
def profile_recall_delta_preview(profile: Mapping[str, Any], delta: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["preview_metadata"] = {
        "generated_by": "scripts/dev/retrieval_v3_discovery_profiles.py",
...
def write_profile(profile: Mapping[str, Any], profile_root: Path) -> Path:
    ...
    if issues:
        raise RetrievalV3DiscoveryProfileError(f"invalid discovery profile: {issues}")
    ...
    *,
    ...
    for path in iter_profile_files(paths, roots):
        ...
        if issues:
            if ignore_invalid_in_roots and str(path.resolve()) not in explicit_paths:
                continue
            raise RetrievalV3DiscoveryProfileError(f"invalid discovery profile {path}: {issues}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage retrieval_v3 discovery profiles.")
    ...
    if args.recall_term_delta:
        if args.output_task_preview:
            if not args.from_task:
                raise RetrievalV3DiscoveryProfileError("--output-task-preview requires --from-task")
        ...
        if len(args.profile) != 1:
            raise RetrievalV3DiscoveryProfileError("--recall-term-delta requires exactly one --profile")
        profile = load_json(args.profile[0])
        issues = validate_profile(profile)
        if issues:
            raise RetrievalV3DiscoveryProfileError(f"invalid discovery profile {args.profile[0]}: {issues}")
...
if __name__ == "__main__":  # pragma: no cover
    ...
    except RetrievalV3DiscoveryProfileError as exc:
