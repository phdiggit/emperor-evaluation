...
def fetch_material_review_items(cur: Any, *, item_code: str, scope: str) -> list[dict[str, Any]]:
    cur.execute(
        ...
        select
          ...
          left join retrieval_v3.claim_rule_bindings crb on crb.id = mrq.binding_id
...
def resolution_payload(row: Mapping[str, Any]) -> dict[str, Any]:
    payload = {
        "source": "retrieval_v3_material_review_consumer",
...
def apply_patch_rows(*, dsn: str, rows: Sequence[Mapping[str, Any]], execute: bool, schema_name: str = "retrieval_v3") -> dict[str, Any]:
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        ...
        if execute:
            ...
            conn.rollback()
    return {
        "generated_by": "scripts/dev/retrieval_v3_material_review_consumer.py",
        "command": "apply-patch",
        "write_db": execute,
        "executed": execute,
        "ok": True,
...
def worklist_report(*, env_file: Path | None, dsn_env: str, item_code: str, scope: str, schema_name: str = "retrieval_v3") -> dict[str, Any]:
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v3 material review consumer report",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Consume retrieval_v3 material review queue patches; dry-run unless --execute.")
