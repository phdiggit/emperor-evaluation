...
def build_rollup(audits: Sequence[tuple[str, Path]]) -> dict[str, Any]:
    ...
    return {
        "generated_by": "scripts/dev/retrieval_v3_claim_quality_rollup.py",
        "ok": True,
        "totals": totals,
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# retrieval_v3 claim quality rollup",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Roll up multiple retrieval_v3 claim audit reports and policy decisions.")
