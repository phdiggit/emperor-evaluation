...
class RetrievalV3TaskSkeletonError(RuntimeError):
    pass
...
def build_task_skeleton(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not target_code or not emperor_name or not rule_code:
        raise RetrievalV3TaskSkeletonError("context requires target_code, emperor_name, and rule_code")
    ...
    return {
        ...
        "source_documents": [],
        "search_plan": {
            "generated_by": "scripts/dev/retrieval_v3_task_skeleton.py",
            "needs_cli_discovery": True,
            "discovery_scope": "object_seeds_and_source_documents",
        },
        "generation_notes": [
            "stable task fields were generated by retrieval_v3_task_skeleton.py; Codex taskgen should only fill discovery fields"
        ],
        "clean_audit": {
            "skeleton_generated_by": "scripts/dev/retrieval_v3_task_skeleton.py",
...
def discovery_prompt(context: Mapping[str, Any], skeleton: Mapping[str, Any], *, allow_search: bool = True) -> str:
    ...
    return (
        "你是 emperor-evaluation retrieval_v3 clean taskgen discovery worker。\n"
