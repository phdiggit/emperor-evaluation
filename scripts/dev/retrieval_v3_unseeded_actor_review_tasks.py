...
def prompt_for_task(task_code: str, workitems: Sequence[Mapping[str, Any]], output_jsonl: Path) -> str:
    ...
    return (
        ...
        "逐条判断 observed_name 是否确为人物名或可唯一指向人物的宗室封号、原文是否把该人物放在目标皇帝的任用/宗室权力/处置语境中，以及当前窗口是实际损害还是只有处置线索。\n"
        "review_verdict 只能是 source_refine、reject_name、needs_context。"
        "source_refine 只表示值得补抓该人物本传、同朝本纪或宗室史源，不表示负向事实成立；皇子、亲王、藩王与其他宗室不得因非臣僚身份排除。\n"
