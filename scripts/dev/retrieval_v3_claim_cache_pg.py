...
def existing_key_counts(cur: Any, rows: Mapping[str, Sequence[Mapping[str, Any]]]) -> dict[str, int]:
    ...
    for table, (column, keys) in specs.items():
        ...
        if not keys:
            counts[table] = 0
            continue
        cur.execute(f"select count(*) as count from retrieval_v3.{table} where {column} = any(%s)", (keys,))
...
def upsert_source_slice(cur: Any, row: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        on conflict (slice_hash) do update set
            ...
            first_run_code = coalesce(nullif(retrieval_v3.claim_source_slices.first_run_code, ''), excluded.first_run_code),
            seen_count = greatest(retrieval_v3.claim_source_slices.seen_count, excluded.seen_count),
            updated_at = now()
        """,
        (
            row["slice_hash"],
...
def upsert_claim(cur: Any, row: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        on conflict (claim_key) do update set
            ...
            first_run_code = coalesce(nullif(retrieval_v3.claim_cache.first_run_code, ''), excluded.first_run_code),
            last_run_code = excluded.last_run_code,
            raw_output_path = excluded.raw_output_path,
            extractor_version = excluded.extractor_version,
            status = case
                when retrieval_v3.claim_cache.status::text in ('rejected', 'superseded') then retrieval_v3.claim_cache.status
                else excluded.status
            end,
            seen_count = greatest(retrieval_v3.claim_cache.seen_count, excluded.seen_count),
            updated_at = now()
        """,
        (
            row["claim_key"],
...
def upsert_evidence(cur: Any, row: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        on conflict (evidence_key) do update set
            ...
            slice_text_preview = excluded.slice_text_preview,
            raw_output_path = excluded.raw_output_path,
            first_run_code = coalesce(nullif(retrieval_v3.claim_evidence.first_run_code, ''), excluded.first_run_code)
        """,
        (
            row["evidence_key"],
            row["claim_key"],
...
def pg_table_counts(cur: Any, *, schema_name: str = DEFAULT_PG_SCHEMA) -> dict[str, int]:
    ...
    for table in ["claim_cache", "claim_source_slices", "claim_evidence", "claim_route_cache", "person_profile_claim_links"]:
        cur.execute(f"select count(*) as count from retrieval_v3.{table}")
...
def pg_owner_scope_inventory(cur: Any) -> dict[str, Any]:
    cur.execute(
        ...
        select owner_scope, count(*) as claim_count
          from retrieval_v3.claim_owner_scopes
...
def pg_inventory(cur: Any, *, sample_limit: int = 0, schema_name: str = DEFAULT_PG_SCHEMA) -> dict[str, Any]:
    ...
    cur.execute(
        ...
        select object_name, count(*) as evidence_count
          from retrieval_v3.claim_evidence
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as raw_cur:
            ...
            if execute:
                cur.execute(
                    f"""
                    delete from retrieval_v3.claim_cache
                     {where_sql}
            ...
            report: dict[str, Any] = {
                "ok": True,
                "generated_by": "scripts/dev/retrieval_v3_claim_cache_pg.py",
        ...
        with conn.cursor() as raw_cur:
            ...
            cur.execute(
                ...
                select s.object_name, count(*) as slice_count
                  from retrieval_v3.claim_source_slices s
                 where not exists (
                       select 1
                         from retrieval_v3.claim_evidence e
            ...
            if execute:
                cur.execute(
                    """
                    delete from retrieval_v3.claim_source_slices s
                     where not exists (
                           select 1
                             from retrieval_v3.claim_evidence e
                            where e.slice_hash = s.slice_hash
...
def inventory_from_pg(*, env_file: Path | None, dsn_env: str, sample_limit: int, schema_name: str) -> dict[str, Any]:
    ...
    report["ok"] = True
    report["generated_by"] = "scripts/dev/retrieval_v3_claim_cache_pg.py"
    return report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import and inspect retrieval_v3 filesystem claim cache in PostgreSQL.")
