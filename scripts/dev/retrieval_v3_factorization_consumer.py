...
from scripts.dev.retrieval_v3_factorization_worklists import (  # noqa: E402
...
def upsert_factor_judgment(
    ...
    *,
    ...
    payload = {
        "source": "retrieval_v3_factorization_consumer",
        "patch": {key: value for key, value in row.items() if not str(key).startswith("_")},
    }
    cur.execute(
        """
        insert into retrieval_v3.claim_rule_binding_factor_judgments (
        ...
        values (
            %s, %s, %s, %s, %s, %s,
            %s, %s, %s, %s::retrieval_v3.rv3_factor_target_action, %s::retrieval_v3.rv3_factor_side, %s::jsonb,
...
def replace_factor_choices(
    ...
    *,
    ...
    cur.execute(
        """
        delete from retrieval_v3.claim_rule_binding_factor_choices
    ...
    for choice in choices:
        ...
        cur.execute(
            """
            insert into retrieval_v3.claim_rule_binding_factor_choices (
            ...
            on conflict on constraint rv3_claim_rule_binding_factor_choices_idem_uk do update set
...
def apply_patch_rows(
    *,
    ...
    return {
        "generated_by": "scripts/dev/retrieval_v3_factorization_consumer.py",
        "command": "apply-patch",
        "write_db": execute,
        "executed": execute,
        "ok": True,
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v3 factorization consumer report",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Consume retrieval_v3 factorization JSONL patches; dry-run unless --execute.")
