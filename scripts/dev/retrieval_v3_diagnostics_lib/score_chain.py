...
from scripts.dev.retrieval_v3_diagnostics_lib.selectors import build_score_chain_selectors, score_chain_params
...
def enrich_score_chain_claim_details(cur: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    rows = fetch_rows(
        ...
        select
            ...
            mc.emperor_name as claim_emperor,
    ...
    def enrich_material(material: Mapping[str, Any], *, target_emperor: str) -> dict[str, Any]:
        enriched = dict(material)
        row = lookup.get(int(enriched.get("claim_id") or 0))
        if not row:
            return enriched
        claim_emperor = text(row.get("claim_emperor")) or text(row.get("emperor_name"))
        if target_emperor and claim_emperor and claim_emperor != target_emperor:
            raise RetrievalV3DiagnosticsError(
                f"cross-emperor score detail lineage: target={target_emperor} "
                f"claim={claim_emperor} claim_id={enriched.get('claim_id')}"
            )
    ...
    for target in targets:
        ...
        enriched_target["materials"] = [
            enrich_material(material, target_emperor=target_emperor)
            for material in target.get("materials") or [] if isinstance(material, Mapping)
        ]
        enriched_target["top_materials"] = [
            enrich_material(material, target_emperor=target_emperor)
            for material in target.get("top_materials") or [] if isinstance(material, Mapping)
        ]
    ...
    *,
    ...
    material_rows = fetch_rows(
        ...
        select
          ...
          join retrieval_v3.retrieval_targets rt on rt.id = ms.target_id
          join scoped_targets st on st.id = ms.target_id
          join retrieval_v3.claim_rule_binding_factor_judgments j on j.id = ms.factor_judgment_id
          join retrieval_v3.claim_rule_bindings b on b.id = ms.binding_id
          join retrieval_v3.material_claims mc on mc.id = ms.claim_id
          left join retrieval_v3.objects o on o.id = ms.object_id
          left join lateral (
                select jsonb_agg(
                    jsonb_build_object(
