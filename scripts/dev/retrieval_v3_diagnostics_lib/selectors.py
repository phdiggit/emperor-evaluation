...
from scripts.dev.retrieval_v3_diagnostics_lib.common import RetrievalV3DiagnosticsError, text
    ...
    *,
    ...
    if selector_names:
        ...
        if normalized_type not in SUPPORTED_SELECTOR_TYPES:
            raise RetrievalV3DiagnosticsError(f"unsupported selector type for score-chain: {normalized_type}")
        if normalized_type == "person" and normalized_role not in SUPPORTED_PERSON_ROLES:
            raise RetrievalV3DiagnosticsError(f"unsupported person selector role for score-chain: {normalized_role}")
