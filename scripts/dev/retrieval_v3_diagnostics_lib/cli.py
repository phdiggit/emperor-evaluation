...
from scripts.dev.retrieval_v3_diagnostics_lib.renderers import write_report
from scripts.dev.retrieval_v3_diagnostics_lib.score_chain import build_score_chain_from_rule_scorer_payload, enrich_score_chain_claim_details
from scripts.dev.retrieval_v3_pg_schema import schema_cursor

def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Read-only diagnostic aggregator for retrieval_v3 consumption.")
    ...
    common.add_argument(
        ...
        type=Path,
        help="For score-chain only: render a retrieval_v3_rule_scorer.py apply JSON payload without reading score tables.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "readiness":
        ...
        raise RetrievalV3DiagnosticsError(f"unsupported command: {args.command}")
