...
def build_plan(*, normalized_root: Path, review_root: Path | None = None, lookup: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    for pack in rows["source_packs"]:
        ...
        if lookup and target_code not in (lookup.get("targets") or {}):
            blockers.append(blocker("source_packs", pack_code, "missing_target", f"target_code not found in retrieval_v2.retrieval_targets: {target_code}"))
        operations.append(
            operation(
                "retrieval_v2.source_packs",
                {"pack_code": pack_code},
                payload={
    ...
    for passage in rows["source_passages"]:
        ...
        operations.append(
            operation(
                "retrieval_v2.source_passages",
                {"document_code": document_code, "raw_passage_code": text(passage.get("raw_passage_code"))},
                depends_on=[{"table": "retrieval_v2.source_documents", "document_code": document_code}],
                payload=passage,
            )
        )
    ...
    for claim in rows["material_claims"]:
        ...
        for passage_ref in claim.get("source_passage_refs") or []:
            ...
            if passage_code in passages:
                operations.append(
                    operation(
                        "retrieval_v2.claim_source_passages",
                        ...
                        depends_on=[
                            ...
                            {"table": "retrieval_v2.source_passages", "passage_code": passage_code},
                        ],
                    )
                )
    ...
    for binding in rows["primary_claim_rule_bindings"]:
        ...
        operations.append(
            operation(
                "retrieval_v2.claim_rule_bindings",
                {
                    "claim_code": claim_code,
                    "rule_code": rule_code,
    ...
    for candidate in rows["claim_rule_binding_candidates"]:
        ...
        operations.append(
            operation(
                "retrieval_v2.claim_rule_binding_candidates",
                {
                    "claim_code": claim_code,
    ...
    for event in rows["coverage_gap_events"]:
        ...
        operations.append(
            operation(
                "retrieval_v2.coverage_gap_events",
                {"idem_key": event_key},
                depends_on=[{"table": "retrieval_v2.source_packs", "pack_code": pack_code}] if pack_code else [],
    ...
    for item in review_rows["object_resolution_worklist"]:
        ...
        operations.append(
            operation(
                "retrieval_v2.object_resolution_queue",
                {"idem_key": "|".join([target_code, text(item.get("item_code")), text(item.get("object_group_key"))])},
                depends_on=[{"table": "retrieval_v2.source_packs", "pack_code": pack_codes[0]}] if pack_codes else [],
                payload={
    ...
    return {
        "generated_by": "scripts/dev/retrieval_v2_import_plan.py",
        "mode": "dry_run_plan",
        "write_db": False,
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v2 import dry-run plan",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a dry-run upsert plan for retrieval_v2 normalized rows.")
