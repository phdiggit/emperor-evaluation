...
def fetch_target_period_gaps(cur: Any, *, item_code: str) -> list[dict[str, Any]]:
    cur.execute(
        ...
        select
          ...
          left join retrieval_v2.person_roles pr
...
def fetch_missing_roles(cur: Any, *, item_code: str) -> list[dict[str, Any]]:
    cur.execute(
        ...
        select
          ...
          join retrieval_v2.retrieval_targets rt on rt.id = tob.target_id
          left join retrieval_v2.material_object_links mol on mol.target_object_id = tob.id
          left join retrieval_v2.person_affiliations pa
    ...
    cur.execute(
        ...
        select
          ...
          left join retrieval_v2.person_roles pr
            on pr.object_id = o.id
           and pr.review_status in ('pending', 'accepted')
          left join retrieval_v2.object_names onm
...
def render_markdown(*, workitems: Sequence[Mapping[str, Any]], tasks: Sequence[Mapping[str, Any]], summary: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v2 judgment worklist",
        "",
...
def write_worklist_outputs(*, output_root: Path, workitems: Sequence[Mapping[str, Any]], batch_size: int) -> dict[str, Any]:
    ...
    summary = {
        "generated_by": "scripts/dev/retrieval_v2_judgment_worklists.py",
        "totals": {
...
def upsert_target_period(cur: Any, row: Mapping[str, Any]) -> None:
    ...
    cur.execute(
        ...
        select o.id as object_id, o.object_identity_key
          from retrieval_v2.retrieval_targets rt
          join retrieval_v2.target_objects tob on tob.target_id = rt.id and tob.object_role = 'target_emperor'
          join retrieval_v2.objects o on o.id = tob.object_id
    ...
    payload = {"source": "retrieval_v2_judgment_patch", "patch": {k: v for k, v in row.items() if not str(k).startswith("_")}}
    cur.execute(
        """
    ...
    cur.execute(
        """
        update retrieval_v2.person_roles
           set dynasty_label = %s,
    ...
    cur.execute(
        """
        update retrieval_v2.person_profiles
           set talent_grade_basis = %s,
               profile_payload = profile_payload || %s::jsonb,
               review_status = 'accepted',
               updated_at = now()
         where object_id = %s
           and coalesce(profile_payload->>'source', '') = 'retrieval_v2_target_person_consumer'
        """,
...
def upsert_person_role(cur: Any, row: Mapping[str, Any]) -> None:
    ...
    cur.execute(
        ...
        values (%s, %s, %s, %s, %s::retrieval_v2.rv2_person_role_kind, %s, %s, %s, 'accepted', %s::jsonb)
...
def update_talent_grade(cur: Any, row: Mapping[str, Any]) -> None:
    ...
    cur.execute(
        """
        update retrieval_v2.person_profiles
           set talent_grade = %s::retrieval_v2.rv2_person_talent_grade,
...
def update_profile_basis(cur: Any, row: Mapping[str, Any]) -> None:
    ...
    payload = {"source": "retrieval_v2_profile_basis_patch", "patch": {k: v for k, v in row.items() if not str(k).startswith("_")}}
    cur.execute(
        """
        update retrieval_v2.person_profiles
           set talent_grade_basis = %s,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build and apply retrieval_v2 consumer judgment worklists.")
