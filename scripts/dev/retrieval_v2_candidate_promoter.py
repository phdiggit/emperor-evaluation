        ...
        select
          ...
          left join lateral (
              select jsonb_object_agg(
                ...
                from retrieval_v2.target_objects tob2
                join retrieval_v2.objects o2 on o2.id = tob2.object_id
...
def build_plan(
    ...
    return {
        "ok": True,
        "generated_by": "scripts/dev/retrieval_v2_candidate_promoter.py",
        "write_db": False,
        "executed": False,
        "totals": {
    ...
    cur.execute(
        ...
        on conflict on constraint rv2_claim_rule_bindings_uk do update set
            usable_for_scoring_cluster = (
                retrieval_v2.claim_rule_bindings.usable_for_scoring_cluster
...
def reconcile_scoring_gates(
    ...
    *,
    ...
    cur.execute(
        f"""
        update retrieval_v2.claim_rule_bindings crb
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# retrieval_v2 candidate promoter",
        "",
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as raw_cur:
            ...
            if execute:
                ...
                if reconciled:
                    executed_counts["retrieval_v2.claim_rule_bindings_scoring_gate"] += reconciled
