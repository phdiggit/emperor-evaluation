...
from scripts.dev.retrieval_v3_bootstrap import (  # noqa: E402
    RetrievalV3BootstrapError,
)
from scripts.dev.retrieval_v3_contracts import SOURCE_HINT_TEXT_ALIASES, alias_script_variants, unique_strings  # noqa: E402
from scripts.dev.retrieval_v3_object_source_cache_seed import (  # noqa: E402
...
from scripts.dev.retrieval_v3_object_source_cache_audit import build_review_audit, merge_rescue_cache, render_review_audit_markdown  # noqa: E402
from scripts.dev.retrieval_v3_object_source_cache_shards import run_build_shards  # noqa: E402
from scripts.dev.retrieval_v3_runtime_paths import default_source_cache_root, load_runtime_paths  # noqa: E402
from scripts.dev.retrieval_v3_source_candidates import (  # noqa: E402
    DEFAULT_CACHE_DIR,
    RetrievalV3CandidateError,
    cache_paths,
    compact_text,
    context_bounds,
    fetch_document_text,
...
from scripts.dev.retrieval_v3_taskgen_preseed import (  # noqa: E402
...
DEFAULT_OUTPUT_ROOT = ROOT / "tmp" / "retrieval_v3_object_source_cache"
SCHEMA_VERSION = 1
...
create table if not exists retrieval_v3.object_source_cache_persons (
    ...
    constraint rv3_object_source_cache_person_name_not_blank check (btrim(person_name) <> ''),
    constraint rv3_object_source_cache_person_priority_positive check (priority > 0)
);

create table if not exists retrieval_v3.object_source_cache_documents (
    document_cache_code text primary key,
    person_cache_code text not null references retrieval_v3.object_source_cache_persons(person_cache_code) on delete cascade,
    ...
    created_at timestamptz not null default now(),
    constraint rv3_object_source_cache_document_title_not_blank check (btrim(source_title) <> '')
);

create table if not exists retrieval_v3.object_source_cache_slices (
    slice_cache_code text primary key,
    document_cache_code text not null references retrieval_v3.object_source_cache_documents(document_cache_code) on delete cascade,
    person_cache_code text not null references retrieval_v3.object_source_cache_persons(person_cache_code) on delete cascade,
    ...
    created_at timestamptz not null default now(),
    constraint rv3_object_source_cache_slice_raw_not_blank check (btrim(raw_text) <> '')
);

create table if not exists retrieval_v3.object_source_cache_coverage (
    person_cache_code text primary key references retrieval_v3.object_source_cache_persons(person_cache_code) on delete cascade,
    ...
    updated_at timestamptz not null default now(),
    constraint rv3_object_source_cache_coverage_agent_status_ck check (
        agent_status in ('not_requested', 'queued', 'running', 'succeeded', 'failed', 'blocked', 'cancelled')
    )
);

create index if not exists rv3_object_source_cache_person_name_idx
on retrieval_v3.object_source_cache_persons(normalized_name, is_emperor);

create index if not exists rv3_object_source_cache_documents_person_idx
on retrieval_v3.object_source_cache_documents(person_cache_code, source_role, source_shape);

create index if not exists rv3_object_source_cache_slices_person_idx
on retrieval_v3.object_source_cache_slices(person_cache_code, document_cache_code);
""".strip() + "\n"
...
def overlay_task_from_cache(
    ...
    *,
    ...
    search_plan["object_source_cache_overlay"] = {
        "generated_by": "scripts/dev/retrieval_v3_object_source_cache.py",
...
def reslice_cache(
    *,
    ...
    elapsed = round(time.perf_counter() - started, 3)
    manifest = {
        "generated_by": "scripts/dev/retrieval_v3_object_source_cache.py reslice",
        "schema_version": SCHEMA_VERSION,
...
def annotate_cache_slices(*, input_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    manifest = {
        "generated_by": "scripts/dev/retrieval_v3_object_source_cache.py annotate-slices",
        "schema_version": SCHEMA_VERSION,
...
def markdown_report(manifest: Mapping[str, Any], coverage_rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    lines = [
        "# retrieval_v3 object source cache report",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build retrieval_v3 object-level source cache artifacts without invoking agents.")
    ...
    seed_db = subparsers.add_parser("seed-from-db", help="Read current retrieval_v3 person objects into seed JSONL.")
    seed_db.add_argument("--output-jsonl", type=Path, required=True)
    seed_db.add_argument("--env-file", type=Path)
    seed_db.add_argument("--dsn-env", default="EMPEROR_EVAL_RETRIEVAL_V3_DSN")
    ...
    overlay = subparsers.add_parser("overlay-task", help="Merge cached object source documents into a retrieval_v3 task JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "overlay-task":
        ...
        if args.output_report_json is not None:
            write_json(args.output_report_json, {"generated_by": "scripts/dev/retrieval_v3_object_source_cache.py", "stats": stats})
...
if __name__ == "__main__":
    ...
    except (ObjectSourceCacheError, ObjectSourceCacheSeedError, RetrievalV3BootstrapError, RetrievalV3CandidateError) as exc:
