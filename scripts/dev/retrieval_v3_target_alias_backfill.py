...
from scripts.dev.retrieval_v3_bootstrap import import_psycopg, load_env_file, normalize_alias, resolve_dsn, seed_target  # noqa: E402
...
from scripts.dev.retrieval_v3_retrieval_target_backfill import fetch_contract, fetch_contract_rule_count  # noqa: E402
...
def build_plan(*, emperor_names: Sequence[str], alias_rows: Sequence[Mapping[str, Any]], schema_name: str) -> dict[str, Any]:
    ...
    return {
        "generated_by": "scripts/dev/retrieval_v3_target_alias_backfill.py",
...
def upsert_target_alias(cur: Any, *, target_id: int, row: Mapping[str, Any]) -> None:
    cur.execute(
        """
        insert into retrieval_v3.target_aliases (
        ...
        on conflict (target_id, alias_type, norm_alias) do update set
            ...
            alias_payload = retrieval_v3.target_aliases.alias_payload || excluded.alias_payload,
            status = 'active'
        """,
        (
            target_id,
        ...
        with conn.cursor() as raw_cur:
            ...
            if execute:
                ...
                for name in emperor_names:
                    ...
                    cur.execute(
                        "select id from retrieval_v3.retrieval_targets where target_code = %s",
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# retrieval_v3 target alias backfill report",
        "",
