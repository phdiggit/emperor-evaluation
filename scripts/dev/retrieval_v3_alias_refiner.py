...
from scripts.dev.retrieval_v3_contracts import alias_script_variants, unique_strings
from scripts.dev.retrieval_v3_source_candidates import alias_entries, object_seed_name
...
class RetrievalV3AliasRefinerError(RuntimeError):
    pass
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RetrievalV3AliasRefinerError(f"expected object JSON: {path}")
    return payload
...
def apply_alias_patches(task: Mapping[str, Any], patches: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    if not isinstance(seeds, list):
        raise RetrievalV3AliasRefinerError("task.object_seeds must be a list")
    ...
    updated["alias_refinement"] = {
        "generated_by": "scripts/dev/retrieval_v3_alias_refiner.py",
...
def build_cli_prompt(task: Mapping[str, Any], patches: Sequence[Mapping[str, Any]]) -> str:
    ...
    return (
        "你是 emperor-evaluation retrieval_v3 alias-refiner worker。\n"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate retrieval_v3 alias refinement patches from coverage gaps.")
...
if __name__ == "__main__":
    ...
    except RetrievalV3AliasRefinerError as exc:
