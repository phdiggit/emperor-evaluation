...
SOURCE = "retrieval_v2_person_context_consumer"
...
def build_context_plan(
    ...
    return {
        "generated_by": "scripts/dev/retrieval_v2_person_context_consumer.py",
        ...
        "operation_counts": {
            "retrieval_v2.person_affiliations": len(affiliation_rows),
            "retrieval_v2.person_roles": len(role_rows),
        },
        "review_needed": {
    ...
    cur.execute(
        ...
        on conflict on constraint rv2_person_affiliations_key_uk do update set
            ...
            affiliation_basis = case
                when btrim(excluded.affiliation_basis) = '' and btrim(retrieval_v2.person_affiliations.affiliation_basis) <> ''
            ...
            review_status = case
                ...
                when retrieval_v2.person_affiliations.review_status = 'accepted'
...
def upsert_person_role(cur: Any, row: Mapping[str, Any], *, affiliation_id: int | None) -> int:
    cur.execute(
        ...
        on conflict on constraint rv2_person_roles_key_uk do update set
            ...
            period_end_year = excluded.period_end_year,
            role_basis = case
                when btrim(excluded.role_basis) = '' and btrim(retrieval_v2.person_roles.role_basis) <> ''
            ...
            review_status = case
                ...
                when retrieval_v2.person_roles.review_status = 'accepted'
                 and excluded.review_status = 'pending'
                    then retrieval_v2.person_roles.review_status
                else excluded.review_status
            end,
...
def markdown_report(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# retrieval_v2 person context consumer report",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Populate retrieval_v2 person role and affiliation candidates from current objects and reference context.")
